sap.ui.define(
    ...
    function (
        ...
        var that, interval;
        return Controller.extend("customActivity.controller.oeeSelectOrderDNA", {
            selectedOrder: {
                orderNumber: "",
                status: "",
                routingOperNo: "",
            ...
            onInit: function () {
                ...
                this.refreshCustomOrderDetails();
                ...
                interval = setInterval(function int() {
                    ...
                    if (!!hash && hash.includes("/")) {
                        hash = hash.split("/");
                        var activityId = hash[hash.length - 1];
                        if (activityId == "ZACT_ORD_DNA")
                            refresh = true;
                    }
                    if (refresh) {
            ...
            refreshCustomOrderDetails: function () {
                ...
                if (
                    ...
                    for (
                        ...
                        activityOptionValuesForDefaultFilters.push({
                            optionValue: this.appData.filtersForManageOrder
            ...
            onClickStart: function (showTimeAndActivity) {
                ...
                var response = TransactionCaller.sync(
                    "MES/Itelli/DNA/ManageOrders/checkSalesOrdersExist",
                    {
                ...
                if (!this.oStatusModel) {
                    ...
                    for (i = 0; i < this.appData.statusList.length; i++) {
                        if (this.appData.statusList[i].status === "ACT") {
                            descriptionOfStatus.Active = this.appData.statusList[
                                i
                            ].description;
                        } else if (this.appData.statusList[i].status === "CMPL") {
                            descriptionOfStatus.Completed = this.appData.statusList[
                                i
                            ].description;
            ...
            handleOK: function (oEvent) {
                ...
                isStartTimeValid = this.validateStartTimeDateForOrder(
                    orderStartResumeTime
                ); // start time in future or not
                    ...
                    if (result === true) {
                        ...
                        if (
                            ...
                            if (
                                ...
                                var selectedProductionActivityContext = sap.ui.core.Fragment.byId(
                                    "prodActivitiesFragment",
                                    "activities"
                                )
                                    .getSelectedItem()
                                    .getBindingContext();
                                selectedProductionActivity = selectedProductionActivityContext.getProperty(
                                    "prodActivity"
                                );
                            }
                            if (
                            ...
                            startDateValueTimeStampInUTC = this.getUTCForCurrentDate(
                                startDateValue
                            );
                            if (
                                ...
                                endDateValueTimeStampInUTC = this.getUTCForCurrentDate(
                                    orderEndResumeTime
                                );
                            ...
                            var startTimeString = sap.oee.ui.Formatter.formatDateTimeToString(
                                startDateValue
                            ); // this is only the time so not converting it to UTC
                            var endTimeString = sap.oee.ui.Formatter.formatDateTimeToString(
                                orderEndResumeTime
                            ); // to set value for this.selectedOrder.endTime , this is only the time so not converting it to UTC
                            ...
                            if (
                                ...
                                var orderStartDate = this.orderStartDateForOverlappingShifts(
                                    startTimeString
                                );
                                if (
                                    orderEndResumeTime !== undefined &&
                ...
                this.ZPP_STATUS_UPDATE(this.selectedOrder.orderNumber, "E0002");
            },
            ...
            onClickHold: function (holdTime) {
                ...
                var endTimeString = sap.oee.ui.Formatter.formatDateTimeToString(
                    endDateValue
                ); // taking out the Time in HH:MM:SS String format
                ...
                if (isStartTimeValid != null && isStartTimeValid === true) {
                    if (
                        ...
                        try {
                            var orderHoldDate = this.orderStartDateForOverlappingShifts(
                                endTimeString
                            );
            ...
            handleOKTime: function (oEvent) {
                ...
                if (workcenterName == "Ark Ocağı" || workcenterName == "Pota Ocağı") {
                    ...
                    var tRunner = new TransactionRunner(
                        "MES/UI/selectOrder/castCloseControl/castCloseControleXquery",
                        params
                    );
                    if (!tRunner.Execute()) {
                        MessageBox.error(tRunner.GetErrorMessage());
            ...
            onClickResume: function (resumeTime) {
                ...
                var resumeTimeString = sap.oee.ui.Formatter.formatDateTimeToString(
                    resumeDateValue
                ); // it is only taking time so not changing it to UTC
                ...
                if (
                    ...
                    var orderResumeDate = this.orderStartDateForOverlappingShifts(
                        resumeTimeString
                    );
                    if (
                        this.appData.node.lineBehavior == "MULTI_CAP_SINGLE_LINE" ||
                ...
                if (this.selectedStatusValue == "ACT") {
                    this.ZPP_STATUS_UPDATE(this.selectedOrder.orderNumber, "E0002");
                }

                if (this.selectedStatusValue == "CMPL") {
                    this.ZPP_STATUS_UPDATE(this.selectedOrder.orderNumber, "E0004");
                }
            ...
            onClickComplete: function (endTime) {
                ...
                var endTimeString = sap.oee.ui.Formatter.formatDateTimeToString(
                    endDateValue
                ); // getting value of End time in HH:MM:SS String format
                // var endTimeStamp_create= new Date(endDateValue).getTime();
                ...
                if (isStartTimeValid != null && isStartTimeValid === true) {
                    if (
                        ...
                        try {
                            var orderCompleteDate = this.orderStartDateForOverlappingShifts(
                                endTimeString
                            );
            ...
            bindOrdersToTable: function () {
                ...
                if (
                    ...
                    orderOperationJSON = this.interfaces.interfacesGetReleasedDemandInputForTimePeriodAndPatternAsync(
                        this.appData.plant,
                        this.appData.node.nodeID,
                        this.appData.client,
                        this.appData.shift.startTimestamp,
            ...
            getTableCharacteristic: function (params) {
                /*
                var response = TransactionCaller.sync("MES/Itelli/DNA/Characteristics/selectOrderCharacteristics/T_GET_CHARACTERISTICS", {
                  I_WORKCENTERID: this.appData.node.workcenterID,
                  I_AUFNRS: JSON.stringify(aufnrParam)
                },
                  "O_JSON"
                );
                if (response[1] == "E") {
                ...
                */
                ...
                var tRunner = new TransactionRunner(
                    "MES/Itelli/DNA/Characteristics/selectOrderCharacteristics/getCharacteristicsSelectOrderQry",
                    params
                );
                if (!tRunner.Execute()) {
                    MessageBox.error(tRunner.GetErrorMessage());
            ...
            renderOrderDetails: function (orderOperationJSON) {
                ...
                var objek = this.getView().getModel("visibleStatusModel").oData[0].OBJEK;
                var aufnrs = "'0000'";
                ...
                var orders1 = orderOperationJSON.orderDetails.results;
                var aufnrs1 = "'0000'";

                for (var i = 0; i < orders1.length; i++) {
                    aufnrs1 = "'" + parseInt(orders1[i].order) + "'" + "," + aufnrs1;
                }
                ...
                var oData = tRunner.GetJSONData();
                // Tablo karakteristikleri 3007
                var aufnrParam = {};
                aufnrParam.AUFNRS = [];
                orders.forEach((item) => {
                    var obj = { AUFNR: item.order };
                    aufnrParam.AUFNRS.push(obj);
                });

                var tableCharacteristic = this.getTableCharacteristic(params1);
                ...
                for (i = 0; i < orderOperationJSON.orderDetails.results.length; i++) {
                    for (k = 0; k < oData[0].Row.length; k++) {
                        if (
                            ...
                            orderOperationJSON.orderDetails.results[i].orderERPGroupNo =
                ...
                if (this.appData.plant == 2001 || this.appData.plant == 3001) {
                    for (i = 0; i < orderOperationJSON.orderDetails.results.length; i++) {
                        if (
                            orderOperationJSON.orderDetails.results[i].orderType == "YP01"
                        ) {
                            orderOperationJSON.orderDetails.results.splice(i, 1);
                            i--;
                        }
                    }
                }
                ...
                if (orderOperationJSON != undefined) {
                    ...
                    if (orderOperationJSON.orderDetails != undefined) {
                        if (
                            ...
                            for (
                                var i = 0;
                                i < orderOperationJSON.orderDetails.results.length;
                                i++
                            ) {
                        ...
                        for (i = 0; i < this.orderDetails.length; i++) {
                            for (k = 0; k < oData[0].Row.length; k++) {
                                if (
                                    ...
                                    this.orderDetails[i].KDPOSS = oData[0].Row[k].KDPOS;
                                    //3007 pcstrand rework için

                                    if (objek == "3007PCSTRAND") {
                                        this.orderDetails[i].AUART = oData[0].Row[k].AUART;
                                    }
            ...
            setOrderOperationData: function (
                ...
                if (
                    ...
                    orderOperationData.action1Label = this.interfaces.oOEEBundle.getText(
                        "OEE_BTN_START"
                    );
                ...
                if (
                    ...
                    if (
                        ...
                        orderOperationData.action3Label = this.productionActivitiesFormatter(
                            operationData.productionActivity
                        );
                        orderOperationData.productionActivity =
                            operationData.productionActivity;
                    } else {
                        orderOperationData.productionActivity = this.interfaces.oOEEBundle.getText(
                            "OEE_BTN_ASSIGN"
                        );
                        orderOperationData.action3Label = this.interfaces.oOEEBundle.getText(
                            "OEE_BTN_ASSIGN"
                        );
                    }
                    orderOperationData.targetAction1 =
                ...
                if (
                    ...
                    orderOperationData.action2Label = this.interfaces.oOEEBundle.getText(
                        "OEE_BTN_COMPLETE"
                    );
                    ...
                    var enableActionButtons = this.hasActiveRunInSelectedShift(
                        operationData
                    );
                ...
                orderOperationData.defaultUomText = this.interfaces.interfacesGetTextForUOM(
                    orderOperationData.quantityReleasedUOM
                ); // amit
            ...
            action1: function (oEvent) {
                ...
                this.selectedOrder.orderNumber = this.orderOperationJSONModel.getProperty(
                    "order",
                    selectedRowContext
                );
                ...
                if (this.selectedOrder.status == "ACT" && workcenterName.includes("Tel Çekme")) {
                    var aufnr = this.selectedOrder.orderNumber;
                    var workcenterID = this.appData.node.workcenterID;

                    var response = TransactionCaller.sync(
                        "MES/Itelli/DNA/CheckRemainingComponents/checkRemainingComponentsTrns",
                        {
                            I_AUFNR: aufnr,
            ...
            callAction1: function (oEvent) {
                ...
                var objek = this.getView().getModel("visibleStatusModel").oData[0]
                    .OBJEK;
                ...
                this.selectedOrder.releasedHeaderID = this.orderOperationJSONModel.getProperty(
                    "releasedHeaderID",
                    selectedRowContext
                );
                this.selectedOrder.releasedID = this.orderOperationJSONModel.getProperty(
                    "releasedID",
                    selectedRowContext
                );
                this.selectedOrder.productionMode = this.orderOperationJSONModel.getProperty(
                    "productionMode",
                    selectedRowContext
                );
                ...
                this.selectedOrder.quantityReleased = this.orderOperationJSONModel.getProperty(
                    "quantityReleased",
                    selectedRowContext
                );
                this.selectedOrder.quantityReleasedUOM = this.orderOperationJSONModel.getProperty(
                    "quantityReleasedUOM",
                    selectedRowContext
                );
                ...
                this.selectedOrder.productionActivity = this.orderOperationJSONModel.getProperty(
                    "productionActivity",
                    selectedRowContext
                );
                this.selectedOrder.numberOfCapacities = this.orderOperationJSONModel.getProperty(
                    "numberOfCapacities",
                    selectedRowContext
                );
                ...
                if (
                    ...
                    this.openEnterTimeDialog(   //açılan fragmenttaki okey onClickResume bu fonk çalıştırıyor Holda alma
                        this.interfaces.oOEEBundle.getText("OEE_LABEL_HOLD_TIME")
                    );
                }

                if (this.selectedOrder.status === sap.oee.ui.oeeConstants.status.HOLD) {
                    this.onResumeOrder(true);   //Holddan aktife alma fragmentı
                }
            ...
            action2: function (oEvent) {
                // aktiften cmplye alırken çalışıyor en altta fragment açılıyor okeye basınca handleOKTime fonk. çalışıyor
            ...
            navigateToOrderDetails: function (oEvent) {
                ...
                this.appData.selected.order.orderNo = this.orderOperationJSONModel.getProperty(
                    "order",
                    selectedRowContext
                );
                this.appData.selected.operationNo = this.orderOperationJSONModel.getProperty(
                    "routingOperNo",
                    selectedRowContext
                );
                this.appData.selected.releasedID = this.orderOperationJSONModel.getProperty(
                    "releasedID",
                    selectedRowContext
                );
                this.router = this.appComponent.getRouter();
            ...
            getCharacteristic: function (workorder) {
                ...
                var response = TransactionCaller.sync(
                    "MES/Itelli/DNA/SDCharList/T_GET_SD_CHAR_LIST",
                    {
                        I_AUFNR: "0000" + workorder,
                    },
                    "O_JSON"
                );
                ...
                if (response[0]?.CharList?.Rowset) {
                    var result = Array.isArray(response[0].CharList.Rowset) ? response[0].CharList.Rowset : new Array(response[0].CharList.Rowset);
                    var currentOrder = 1;
                    result.forEach((arr) => {
                        if (!!arr?.Row) {
                            var currentArr = Array.isArray(arr.Row) ? arr.Row : new Array(arr.Row);
                            if (currentArr.length > 0) {
                                var merkezCevre = currentArr[0].MERKEZ_CEVRE;
                                var headerText = "";
                                if (merkezCevre == "---") {
                                    headerText = `${currentArr[0].MALZEME_SEVIYESI}`;
                                }
                                else {
                                    headerText = `${currentArr[0].MALZEME_SEVIYESI}(${merkezCevre})`;
                                }
                            }
                            var myModel = new sap.ui.model.json.JSONModel();
                            myModel.setData(currentArr);
                            this.getView().byId(`panel${currentOrder}`).setHeaderText(headerText);
                            this.getView().byId(`charTable${currentOrder}`).setModel(myModel);
                        }
                        currentOrder++;
                    });
                    if (currentOrder < 9) {
                        for (i = currentOrder; i <= 9; i++) {
                            this.getView().byId(`panel${i}`).setVisible(false);
                        }
                    }
                }
            ...
            onPressOpenCharacteristic: function (oEvent) {
                ...
                if (!oDialog) {
                    ...
                    oDialog = sap.ui.xmlfragment(
                        oView.getId(),
                        "customActivity.fragmentView.getCharacteristicOrderDNA",
                        this
                    );
            ...
            onPressSaveChanges: function (oEvent) {
                ...
                var tableData = this.getView().byId("continuousCastingTable").getModel()
                    .oData;
            ...
            reworkOrder: function (oEvent) {
                ...
                var orderNumber = parseFloat(oTableData.orderDetails[selectedIndex].order);

                this.appComponent.getRouter().navTo("activity", {
                    activityId: "ZACT_REP_QTY_DHS_R",
                    mode: orderNumber,
                });
            ...
            onPressCmpltoHold: function (oEvent) {
                ...
                if (this.selectedOrder.status == "ACT" && this.appData.node.description.includes("Tel Çekme")) {
                    var aufnr = this.selectedOrder.orderNumber;
                    var workcenterID = this.appData.node.workcenterID;
                ...
                var operNo = oTableData.orderDetails[selectedIndex].routingOperNo;
