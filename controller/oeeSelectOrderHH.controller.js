sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("customActivity.controller.oeeSelectOrderHH", {
      selectedOrder: {
        orderNumber: "",
        status: "",
        routingOperNo: "",
      ...
      /*
         autoPauseOrder:function(){
                   var nodeId = this.appData.node.nodeID;
                    var params = {
                      I_NODEID: nodeId
                    };
                    var tRunner = new TransactionRunner(
                     "MES/UI/selectOrder/orderStatusUpdate/autoPauseOrderXqry",
                      params
                    );
             if(!tRunner.Execute()){
                              MessageBox.show(tRunner.GetErrorMessage(), "E");
                              return {};
                          };
        },
      ...
      */
        ...
        if (workcenterName == "Ark Ocağı" || workcenterName == "Pota Ocağı") {
          ...
          if (valueStateDate == "None" && valueStateTime == "None") {
            try {
              ...
              if (this.customData.getValue() == "COMPLETE") {
                //agemici - DHSH-5939
                for (var i = 0; i < this.getView().byId("idOrdersTable").getModel().getData().orderDetails.length; i++) {
                  if (aufnr == this.getView().byId("idOrdersTable").getModel().getData().orderDetails[i].order) {
                    break;
                  }
                }

                if (this.getView().byId("idOrdersTable").getModel().getData().orderDetails[i].status == sap.oee.ui.oeeConstants.status.HOLD) {
                  this.onClickComplete_HoldOrder(
                    this.selectedOrder.orderNumber,
                    this.appData.node.nodeID,
                    this.getView().byId("idOrdersTable").getModel().getData().orderDetails[i].routingOperNo,
                    combinedDateTime);
                } else {
                  this.onClickComplete(combinedDateTime);
                }
      ...
      getTableCharacteristic: function (params) {
        var tRunner = new TransactionRunner(
          "MES/UI/selectOrder/getCharacteristicColumnsHHQry",
          params
        );
        if (!tRunner.Execute()) {
          MessageBox.error(tRunner.GetErrorMessage());
            ...
            if (
              ...
              for (var i = 0; i < this.orderDetails.length; i++) {
                if (jsData[0].Row != undefined) {
                  for (var k = 0; k < jsData[0].Row.length; k++) {
                    if (this.orderDetails[i].order == jsData[0].Row[k].AUFNR) {
                      if (jsData[0].Row[k - 1] != undefined && jsData[0].Row[k].AUFNR == jsData[0].Row[k - 1].AUFNR) {
                        this.orderDetails[i].material = jsData[0].Row[k].MATNR + " / " + jsData[0].Row[k - 1].MATNR;
                        this.orderDetails[i].materialDescription = jsData[0].Row[k].MAKTX + " / " + jsData[0].Row[k - 1].MAKTX;
                      } else {
                        this.orderDetails[i].material = jsData[0].Row[k].MATNR;
                        this.orderDetails[i].materialDescription = jsData[0].Row[k].MAKTX;
                      }
      ...
      //agemici - DHSH-5939
      onClickComplete_HoldOrder: function (orderNo, nodeID, operationNo, endTime) {
        var response = TransactionCaller.sync("MES/UI/Haddehane/Operations/CompleteOrder_onHold/T_Complete_Order_OnHold", {
          I_AUFNR: orderNo,
          I_NODEID: nodeID,
          I_VORNR: operationNo,
          I_ENDTIME: endTime
