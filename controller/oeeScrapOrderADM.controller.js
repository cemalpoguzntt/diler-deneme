sap.ui.define(
    ...
    function (
        ...
        return Controller.extend("customActivity.controller.oeeScrapOrderADM", {
            /**
            ...
            onInit: function () {
                ...
                //     this.getView().byId("cumquan").setVisible(true);
                //     this.getView().byId("scrQuan").setEnabled(true);
                // }else if(this.appData.plant == "2001"){
                //     this.getView().byId("cumquan").setVisible(true);
                //     this.getView().byId("scrQuan").setEnabled(true);
                // }
                ...
                var hbiAuth = [{PLANT : this.appData.plant}];
                var oModelHBI = new sap.ui.model.json.JSONModel();
                oModelHBI.setData(hbiAuth);
                this.getView().setModel(oModelHBI, "HBIAUTH");
            ...
            callStatusList: function (p_this, p_data) {
                var cu_length = p_data.Rowsets.Rowset[0].Row.length;
                var cu_plant = p_this.appData.plant;
                for (var i = 0; i < cu_length; i++) {
                    if(cu_plant == '2001' && p_data.Rowsets.Rowset[0].Row[i].STATUSCODE == '260'){
                        var cu_data = p_data.Rowsets.Rowset[0].Row[i];
                    } else if(cu_plant == '3001' && p_data.Rowsets.Rowset[0].Row[i].STATUSCODE == '360'){
                        var cu_data = p_data.Rowsets.Rowset[0].Row[i];
                    }
                }
                var oModel = new sap.ui.model.json.JSONModel();
                oModel.setData([cu_data]);
                p_this.getView().byId("statusSelect").setModel(oModel)
              //  p_this.getView().setModel(oModel, "statusList");
                //p_this.getVisibleStatus(p_data.Rowsets.Rowset[0].Row[0].STATUSCODE);
                p_this.getMasterDetailList(cu_data.STATUSCODE);
                p_this.getCastingNumberList(cu_data.STATUSCODE);
                p_this.screenObj.statusSelect = cu_data.STATUSCODE;
            },
        ...
        */
            ...
            saveChargeScrap: function (oEvent) {
                ...
                var tRunner2 = new TransactionRunner(
                        "MES/UI/ScrapOrder/Admin/getCastNoForControl",
                        params
                );
                if (!tRunner2.Execute()) {
                ...
                var cuData = tRunner2.GetJSONData();
                if(cuData[0].Row != undefined){
                    MessageBox.error("Döküm kapatıldığı için kayıt yapamazsınız, sayfayı yenileyiniz.");
                    return false;
                }
                //Döküm Kapalı Kontrolü
