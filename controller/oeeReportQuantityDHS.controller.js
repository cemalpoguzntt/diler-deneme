sap.ui.define(
    ...
    function (
        ...
        return Controller.extend("customActivity/controller/oeeReportQuantityDHS", {
                ...
                if (objek == "3007FOSFAT") {
                    ...
                    if (
                        !(
                            ...
                            quantityReleased + quantityReleased * 0.04
                        )
                    ) {
                        MessageBox.error(
            ...
            callPcStrandQry: function (p_data, p_this) {
                ...
                oData.forEach(function (item, index) {
                    ...
                    oData[index].Y_CAP_PCS_MM = Number((oData[index]?.Y_CAP_PCS_MM)?.replace(',', '.'))?.toFixed(2);
            ...
            onSavePcStrand: function (oEvent) {
                ...
                if (isSavePcStrand) {
                    ...
                    var selectedContextPath = this.getView().byId("pcStrandDialogTable").getSelectedContextPaths("rows")[0];
                    var selectedLine = this.getView().byId("pcStrandDialogTable").getModel().getObject(selectedContextPath);
                    ...
                    var selectedWeight = this.getView().byId("pcStrandDialogTable").getSelectedItem().mAggregations.cells[3].mProperties.text;
                    ...
                    //  DHS Şifreli Hurda Girişi
                    var params = {
                        "Param.1": "DHS_REP_QNT_HURDA_PSW",
                    };
                    ...
                    var DHS_REP_QNT_HURDA_PSW = Number(oCNTData[0].Row[0].VALUE);


                    //  DHS Hurda Girişi
                    var params2 = {
                        "Param.1": "DHS_REP_QNT_HURDA",
                    };
                    ...
                    var DHS_REP_QNT_HURDA = Number(oCNTData2[0].Row[0].VALUE);


                    if (oData[0].checkBox && scrapQuantity > DHS_REP_QNT_HURDA_PSW) {

                        //  DHS Hurda Girişi Şifresi
                        var params = {
                            "Param.1": "DHS_REP_QNT_HURDA_ENTRY_PSW",
                        };
                        var tRunner = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params);
                        if (!tRunner.Execute()) {
                            MessageBox.error(tRunner.GetErrorMessage());
                        ...
                        var adminPassword = prompt(DHS_REP_QNT_HURDA_PSW + " kg üstünde hurda girişi yapmak için şifre gereklidir.")

                        if (adminPassword == value) {
                            this.onSavePcStrand2(oEvent);
                        } else {
                            isSavePcStrand = true;
                            return;
                        }

                    } else if (!oData[0].checkBox && scrapQuantity < DHS_REP_QNT_HURDA) {
            ...
            onSavePcStrand2: function (oEvent) {
                ...
                TransactionCaller.async(
                    "MES/Itelli/DNA/ReportProduction/DHS/WeightSource/updateWeightSourceTrns",
                    {
                        I_BATCH: this.byId(batchInput).getValue(),
                        I_WEIGHTSOURCE: this.productionType
                ...
                var responseKGM = TransactionCaller.sync(
                    "MES/Itelli/DNA/BabyCoilMeterage/getMasterCoilKGM",
                    {
                        I_AUFNR: aufnr,
                        I_MC_BATCH: selectedLine.BARCODE,
                        I_NODE_ID: nodeID
                    },
                    "O_JSON"
                );
                if (responseKGM[1] == "E") {
                    MessageBox.error(responseKGM[0]);
                    isSavePcStrand = true;
                    return;
                } else {
                    var calculatedMeterage = 0;
                    calculatedMeterage = parseInt(currentWeight / responseKGM[0]);
                    this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[1].setValue(calculatedMeterage)
                }
                if (objek == "3007PCSTRAND") {
                    this.screenObj.inputChar.push({
                        CHAR_NAME: "Y_METRAJ",
                        CHAR_VALUE: calculatedMeterage,
                    });
                }
                if (objek == "3007PCSTRAND") {
                    this.screenObj.inputChar.push({
                ...
                if (objek == "3007PCSTRAND") {  //Update Row Number
                ...
                if (oData[0].checkBox) {
                    ...
                    oData[oData.length] = {};
                    oData[oData.length - 1].QUANTITY = (parseFloat(oData[0].QUANTITY) - parseFloat(weightValue)).toFixed(3);
            ...
            onPressGetWeightManuel: function (oEvent) {

                var params = {
                    "Param.1": "PCS_REP_QNT_MANUEL_WEI_PSW",
                };
                var tRunner = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params);
                if (!tRunner.Execute()) {
                    MessageBox.error(tRunner.GetErrorMessage());
                ...
                var oData = tRunner.GetJSONData();
                var PCS_REP_QNT_MANUEL_WEI_PSW = oData[0].Row[0].VALUE;

                var adminPassword = prompt("Manuel tartım ile ilerlemek için lütfen şifreyi girin")
                if (adminPassword != PCS_REP_QNT_MANUEL_WEI_PSW) {
                    MessageBox.error("Girilen şifre hatalı!");
                    return;
                } else {
                    this.productionType = "MANUEL";
            ...
            updateWeightCB: function (iv_data, iv_scope) {
                if (iv_data[1] == "E") {
                    MessageBox.error(iv_data[0]);
                }
