sap.ui.define(
    ...
    function (
        ...
        return Controller.extend(
            "customActivity.controller.oeeGenericDataCollectionFilter",
            {
                onInit: function () {
                    this.appComponent = this.getView().getViewData().appComponent;
                    this.appData = this.appComponent.getAppGlobalData();
                    this.interfaces = this.appComponent.getODataInterface();
                    this.appComponent.getEventBus().subscribe(this.appComponent.getId(), "orderChanged", this.renderUI, this);
                    this.bindDataToCard();
                    this.renderUI();
                ...
                renderUI: function () {
                    ...
                    if (!this.appData.anyOrderRunningInShift()) { // Do not proceed if not.
                        return;
                    }
                    ...
                    if (this.getView().getViewData() != undefined) {
                        ...
                        if (viewOptions != undefined && viewOptions.length > 0) {
                            for (var i = 0; i < viewOptions.length; i++) {
                                if (viewOptions[i].optionName == "CONTEXT") {
                                    if (viewOptions[i].activityOptionValueDTOList != undefined) {
                                        if (viewOptions[i].activityOptionValueDTOList.results != undefined) {
                                            var contextOptions = viewOptions[i].activityOptionValueDTOList.results;
                                    ...
                                    if (viewOptions[i].activityOptionValueDTOList != undefined) {
                                        if (viewOptions[i].activityOptionValueDTOList.results != undefined) {
                                            var contextOptions = viewOptions[i].activityOptionValueDTOList.results;
                                            for (var cIndex = 0; cIndex < contextOptions.length; cIndex++) {
                                                dcElements.push({ client: this.appData.client, dcElement: contextOptions[cIndex].optionValue });
                                            }
                                        }
                                    }
                            ...
                            sap.oee.ui.Utils.createMessage(this.appComponent.oBundle.getText("OEE_ERR_MSG_NO_CONFIG"), sap.ui.core.MessageType.Error);
                            return;
                        }
                    } else {
                        sap.oee.ui.Utils.createMessage(this.appComponent.oBundle.getText("OEE_ERR_MSG_NO_CONFIG"), sap.ui.core.MessageType.Error);
                    ...
                    var response;
                    if (context) {
                        response = this.interfaces.getNonSAPDeliveredDCElementsForContext(this.appData.client, context);
                    } else {
                        response = this.interfaces.getDCElementDetails(dcElements);
                    }
                    if (response && response.dataCollectionElements && response.dataCollectionElements.results) {
                        for (var i = 0; i < response.dataCollectionElements.results.length; i++) {
                    ...
                    var contextBased = this.interfaces.interfacesGetTotalQuantityCollectedForDCElementsInBaseUom(dcElements);
                    if (contextBased && contextBased.prodRunDataList != undefined && contextBased.prodRunDataList.results != undefined)
                        if (contextBased.prodRunDataList.results.length > 0) {
                            for (var i = 0; i < contextBased.prodRunDataList.results.length; i++) {
                                var tempProdRunData = contextBased.prodRunDataList.results[i];
                                contextBased.prodRunDataList.results[i].quantityReported = tempProdRunData.quantityInStdRateUom;
                                contextBased.prodRunDataList.results[i].assignRCLink = this.appComponent.oBundle.getText("OEE_LABEL_ASSIGN");
                                for (var j = 0; j < tempDCElements.length; j++) {
                                    if (tempDCElements[j].dcElement == contextBased.prodRunDataList.results[i].dcElement) {
                                        contextBased.prodRunDataList.results[i].dcElementType = tempDCElements[j].dcElementType;
                                        contextBased.prodRunDataList.results[i].defaultUOMText = this.interfaces.interfacesGetTextForUOM(tempDCElements[j].reportingUom);
                                        contextBased.prodRunDataList.results[i].defaultUOM = tempDCElements[j].reportingUom;
                                        contextBased.prodRunDataList.results[i].description = tempDCElements[j].description;
                                        contextBased.prodRunDataList.results[i].lossType = false;
                                        if (tempDCElements[j].timeElementcategory == sap.oee.ui.oeeConstants.timeElementCategoryForLoss) {
                                ...
                                contextBased.prodRunDataList.results[i].timeStampOfSelectedStartTime = "";
                                contextBased.prodRunDataList.results[i].timeStampOfSelectedEndTime = "";
                                contextBased.prodRunDataList.results[i].lastReportedTimestamp = tempProdRunData.changeTimestamp;
                ...
                dataPreparationAndBinding: function (oContext) {
                    ...
                    for (i = 0; i < oContext.prodRunDataList.results.length; i++) {
                        ...
                        if (oContext.prodRunDataList.results[i].dcElementType == this.appData.constDCElementTypeForQuantity) {
                            oContext.prodRunDataList.results[i].rowIndex = contextQuantityRowIndex;
                            this.oContextQuantityType.push(oContext.prodRunDataList.results[i]);
                    ...
                    this.getView().byId("commentColumnForQuantityType").setVisible(reasonCodeColumnVisibilityForQuantityType);
                    this.getView().byId("reasonCodeColumnForQuantityType").setVisible(reasonCodeColumnVisibilityForQuantityType);

                    if (this.oContextQuantityType != undefined) {
                        if (this.oContextQuantityType.length > 0) {
                            contextDataModelForQuantityType.setData({ genericDataCollectionDataQuantityType: this.oContextQuantityType });
                            this.byId("reportGenericDataCollectionTableQuantityType").setModel(contextDataModelForQuantityType);
                        }
                        else if (this.oContextQuantityType.length == 0) {
                            this.getView().byId("reportGenericDataCollectionTableQuantityType").setVisible(false);
                        }
                    }
                },
                ...
                handleValueHelpRequest: function (oEvent) {
                    ...
                    if (this.oContextQuantityType != undefined && this.oContextQuantityType.length > 0) {
                        for (i = 0; i < this.oContextQuantityType.length; i++) {
                            if (oEvent.getSource().getBindingContext().getObject().dcElement == this.oContextQuantityType[i].dcElement) {
                                break;
                            }
                            oIndex++;
                        }
                        var oResults = this.interfaces.findAllUomsForAGivenUOM(this.appData.client, this.oContextQuantityType[oIndex].defaultUOM);
                        if (oResults != undefined && oResults.uomList && oResults.uomList.results) {
                        ...
                        if (this.oUomDialog == undefined) {
                            this.oUomDialog = sap.ui.xmlfragment("sap.oee.ui.fragments.UOMPopup", this);
                ...
                uomSearch: function (oEvent) {
                    ...
                    properties.push("uom");
                    sap.oee.ui.Utils.uomSearch(oEvent.getSource()._oSearchField, this.oUomDialog.getModel(), this.oUomDialog.getBinding("items"), properties);
                },
                ...
                handleSelect: function (oEvent) {
                    ...
                    if (this._inputFieldForWhichValueHelpRequested != undefined) {
                        this._inputFieldForWhichValueHelpRequested.getBindingContext().getObject().defaultUOM = sUom;
                        this._inputFieldForWhichValueHelpRequested.getBindingContext().getObject().defaultUOMText = sUomText;
                ...
                handleCancel: function (oEvent) {
                    if (this.detailsDialog != undefined) {
                        sap.ui.getCore().byId(sap.ui.core.Fragment.createId("detailsFragment", "deleteButton")).setEnabled(false);
                    }
                ...
                onPressAddComments: function (oEvent) {
                    this.selectedRowIndex = oEvent.getSource().getBindingContext().getObject().rowIndex;
                    var oIndex = 0;
                    if (oEvent.getSource().getBindingContext().getObject().dcElementType == this.appData.constDCElementTypeForQuantity) {
                        for (i = 0; i < this.oContextQuantityType.length; i++) {
                            if (oEvent.getSource().getBindingContext().getObject().dcElement == this.oContextQuantityType[i].dcElement) {
                                break;
                            }
                    ...
                    if (this.oDialogComment == undefined) {
                        this.oDialogComment = sap.ui.xmlfragment("commentPopup",
                            "sap.oee.ui.fragments.commentPopup", this);
                        this.getView().addDependent(this.oDialogComment);
                    }
                    var commentBox = sap.ui.getCore().byId(sap.ui.core.Fragment.createId("commentPopup", "comment"));
                    commentBox.setValue(""); // Clear 
                    if (oResults.comments != "") {
                        sap.ui.getCore().byId(sap.ui.core.Fragment.createId("commentPopup", "comment")).setValue(oResults.comments);
                    }
                ...
                onCommentDialogSaveButton: function (oEvent) {
                    ...
                    if (this.oContextObject.dcElementType == this.appData.constDCElementTypeForQuantity) {
                        sap.oee.ui.Utils.updateModel(this.byId("reportGenericDataCollectionTableQuantityType").getModel());
                    }
                    this.oDialogComment.close();
                },
                ...
                onClickReasonCode: function (oEvent) {
                    ...
                    sap.oee.ui.rcUtility.createReasonCodeToolPopup(this,
                        reasonCodeLink,
                        this.appData.client,
                        this.appData.plant,
                        this.appData.node.nodeID,
                ...
                onPressDetails: function (oEvent) {
                    ...
                    this.appComponent.getEventBus().publish(this.appComponent.getId(), "openDetailsHandler", oData);
                },
                ...
                onClickAddComments: function (oEvent) {
                    ...
                    if (this.oDialogComment == undefined) {
                        this.oDialogComment = sap.ui.xmlfragment("commentPopup", "sap.oee.ui.fragments.commentPopup", this);
                        this.getView().addDependent(this.oDialogComment);
                    }
                    var commentBox = sap.ui.getCore().byId(sap.ui.core.Fragment.createId("commentPopup", "comment"));
                    commentBox.setValue(""); // Clear 
                    if (oContextObject.comments != "") {
                        sap.ui.getCore().byId(sap.ui.core.Fragment.createId("commentPopup", "comment")).setValue(oContextObject.comments);
                    }
                    this.oDialogComment.open();
                },
                ...
                reportData: function (oEvent) {
                    ...
                    if (this.oContextQuantityType != undefined && this.oContextQuantityType.length > 0) {
                        for (var index = 0; index < this.oContextQuantityType.length; index++) {
                            if (this.oContextQuantityType[index].quantityReportedNew != "") {
                                ...
                                orderIndependentData.dcElementType = sap.oee.ui.oeeConstants.dcElementTypeForOtherQuantity;
                                orderIndependentData.dcElement = this.oContextQuantityType[index].dcElement;
                                orderIndependentData.quantity = this.oContextQuantityType[index].quantityReportedNew;
                                if (!sap.oee.ui.Utils.isQuantityValidForLocale(orderIndependentData.quantity)) {
                                    return;
                                }
                                else {
                                    orderIndependentData.quantity = floatFormatter.parse(orderIndependentData.quantity);
                                }
                                orderIndependentData.comments = this.oContextQuantityType[index].comments;
                                orderIndependentData.uom = this.oContextQuantityType[index].defaultUOM;
                                if (this.oContextQuantityType[index].reasonCodeData != undefined) {
                                    if (this.oContextQuantityType[index].reasonCodeData != "") {
                                        orderIndependentData.rc1 = this.oContextQuantityType[index].reasonCodeData.reasonCode1;
                                        orderIndependentData.rc2 = this.oContextQuantityType[index].reasonCodeData.reasonCode2;
                                        orderIndependentData.rc3 = this.oContextQuantityType[index].reasonCodeData.reasonCode3;
                                        orderIndependentData.rc4 = this.oContextQuantityType[index].reasonCodeData.reasonCode4;
                                        orderIndependentData.rc5 = this.oContextQuantityType[index].reasonCodeData.reasonCode5;
                                        orderIndependentData.rc6 = this.oContextQuantityType[index].reasonCodeData.reasonCode6;
                                        orderIndependentData.rc7 = this.oContextQuantityType[index].reasonCodeData.reasonCode7;
                                        orderIndependentData.rc8 = this.oContextQuantityType[index].reasonCodeData.reasonCode8;
                                        orderIndependentData.rc9 = this.oContextQuantityType[index].reasonCodeData.reasonCode9;
                                        orderIndependentData.rc10 = this.oContextQuantityType[index].reasonCodeData.reasonCode10;
                                    }
                                }
                                ...
                                if (currentTimeStamp < shiftEndTimeStamp) {
                                    ...
                                    var mostRecentReportingTime = sap.oee.ui.Utils.getMostRecentReportingTime(this.appData.shift.startTimestamp, this.appData.shift.endTimestamp, this.appData.selected.startTimestamp);
                ...
                updateLastReportedDateTime: function (dataReported) {
                    var oTime = this.getTimeFromMillisecondsTo12HourFormat(new Date(new Date().toDateString() + " " + new Date().toTimeString().split(" ")[0]).getTime());
                    var oDate = new Date().toDateString();
                    for (i = 0; i < dataReported.length; i++) {
                        if (this.oContextQuantityType != undefined && this.oContextQuantityType.length > 0) {
                            for (index = 0; index < this.oContextQuantityType.length; index++) {
                                if (this.oContextQuantityType[index].dcElement == dataed[i].dcElement) {
                                    this.oContextQuantityType[index].lastReportedDateTime = oDate + " " + oTime;
                                }
                            }
                            sap.oee.ui.Utils.updateModel(this.byId("reportGenericDataCollectionTableQuantityType").getModel());
                        }
                ...
                onExit: function () {
                    ...
                    this.appComponent.getEventBus().unsubscribe(this.appComponent.getId(), "orderChanged", this.renderUI, this);
                },
