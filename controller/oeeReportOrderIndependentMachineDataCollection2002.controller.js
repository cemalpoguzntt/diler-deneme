sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "customActivity.controller.oeeReportOrderIndependentMachineDataCollection2002",
      {
        onInit: function () {
          this.appComponent = this.getView().getViewData().appComponent;
          this.appData = this.appComponent.getAppGlobalData();
          this.interfaces = this.appComponent.getODataInterface();
          this.getCustomDcElementData();
        ...
        getCustomDcElementData: function () {
          var nodeID = this.appData.node.nodeID;
          var params = { "Param.1": nodeID };
          var tRunner = new TransactionRunner(
            "MES/UI/ReportOrderIndependentMachineDataCollection2002/getCustomDcElementDataQry",
            params
          );
          if (!tRunner.Execute()) {
          ...
          this.customDcElementData = oData[0].Row;
        },
        renderUI: function () {
          /*
        ...
        dataPreparationAndBinding: function (oContext) {
                var tRunner = new TransactionRunner(
                    "MES/UI/Filmasin/Energy/getFlmEnergyDatasXqry"
                ); 
            if (!tRunner.Execute()) {
              MessageBox.error(tRunner.GetErrorMessage());
              return null;
          ...
          this.customDcElementData.forEach(function (item, index) {
            for (var i = 0; i < this.oContextQuantityType.length; i++) {
              if (item.VALUE == this.oContextQuantityType[i].dcElement)
                customTableData.push(this.oContextQuantityType[i]);
            }
          }, this);
          if (this.oContextQuantityType != undefined) {
            if (this.oContextQuantityType.length > 0) {
              contextDataModelForQuantityType.setData({
                genericDataCollectionDataQuantityType: customTableData,
              });
      for (
                  i = 0;
                  i <
                  ...
                  for (k = 0; k < oData[0].Row.length; k++) {
                   var value = oData[0].Row[k].VALUE + "";
                   var changedValue =  JSON.parse(JSON.stringify(value).replaceAll(".", ","));
                    if (
                      contextDataModelForQuantityType.oData
                        .genericDataCollectionDataQuantityType[i].dcElement ==
                      oData[0].Row[k].NAME
                    )                     
                      contextDataModelForQuantityType.oData.genericDataCollectionDataQuantityType[
                        i
                      ].quantityReported = changedValue;
                  }
                }

                
  /* 
             this.dcElementsData.dataCollectionElements.results[30].quantityReported
= data.FLM_DSU;
              this.dcElementsData.dataCollectionElements.results[31].quantityReported
= data.FLM_GNL_IDR;
              this.dcElementsData.dataCollectionElements.results[32].quantityReported
= data.FLM_ENERGY;           
            },
*/
