sap.ui.define(
    ...
    function (
        ...
        var productionTypeWeight;
        var productionTeoWeight;
        var prodTypeSCT;
        var pastRunId;

        return Controller.extend("customActivity/controller/oeeReportQuantitySCT", {
            ...
            modelServices: function () {
                ...
                oTrigger.addListener(() => {
                    if (this.appData.intervalState) {
                        this.getBobbinNo();   //intervalde çalışmasını istediğimiz fonksiyonu buraya yazıyoruz
                        this.PCoMetraj();
                        this.capToleransCalculation();
                        this.getRowNumber();
                        this.getProductionData();
                    }
                    console.log("bobin interval çalışıyor");
                }, this);
            },
            checkShift: function () {
            ...
            onPressResetPartiNo: function (oEvent) {
                ...
                this.byId("reportProductionQuantityTable").mAggregations.items[1].mAggregations.cells[3].setValue(null);
                this.byId("reportProductionQuantityTable").mAggregations.items[1].mAggregations.cells[1].setValue(null);
                this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[4].setSelectedKey(null);
                this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[5].setSelectedKey(null);
                ...
                if (objek == "3007FOSFAT") {
                    ...
                    if (
                        !(
                            ...
                            quantityReleased + quantityReleased * 0.10
                        )
                    ) {
                        MessageBox.error(
                    ...
                    if (goodQuantityBatch != "") {
                        var measuredDiameter = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[4].getSelectedKey();
                        var selectedBobbinNo = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[5].getSelectedKey();

                        if (selectedBobbinNo == '') {
                            MessageBox.show("Bobin seçimi zorunludur");
                            return;
                        }
            ...
            callComponentDetails: function (p_this, p_data) {
                ...
                if (!!rows && rows?.length > 0) {
                    ...
                    if (prodTypeSCT == "GOOD_QUANTITY") {
                        p_this.getView().byId("idProductionType").setVisible(true);
                        p_this.getView().byId("idHeader").setVisible(true);

                    } else if (prodTypeSCT == "SCRAP") {
                        p_this.getView().byId("idProductionType").setVisible(false);
                        p_this.getView().byId("idHeader").setVisible(false);
                    }

                    var myModel = p_data.Rowsets.Rowset[0].Row;

                    myModel.forEach(function (item, index) {

                        myModel[index].Y_CAP_FLM_MM = Number((myModel[index].Y_CAP_FLM_MM).replace(',', '.')).toFixed(2);
                    ...
                    if (prodTypeSCT == "GOOD_QUANTITY") {
                        p_this.onChangeProdunctionType();
                    }
                    else if (prodTypeSCT == "SCRAP") {
                        p_this.sortConsumptionOrderScrap();
                    }
            ...
            getConfirmInformation: function () {
                ...
                var tRunner = new TransactionRunner(
                    "MES/UI/ReportQuantity/getConfirmInformationSCTQry",
                    params
                );
            ...
            getBatchNumber: function (type, batchNumber, rowNumber) {
                ...
                var filterModel = oData[0]?.Row?.filter((i) => (i.PROD_TYPE == selectKey));
                if (filterModel != undefined) return filterModel;
                else return false;
            },
            handleAddConfirm: function (oEvent) {
                ...
                var model = this.getView().byId("reportProductionQuantityTable")?.getModel()?.oData?.productionData;

                model.forEach(function (item, index) {
                    if (!!model[index].batchNumber) {
                        MessageBox.error("Mevcut parti varken yeni bir parti oluşturulamaz!");
                        return controlforEach = true;
                    }
            ...
            onSaveRecords: function (oEvent) {
                if (isSaveRecords) {
                    ...
                    var selectedProdType = this.getView().byId("selectProductionType").getSelectedKey();

                    //  SCT Teorik Top Limiti <1000	
                    var params1 = {
                        "Param.1": "SCT_REP_QNT_TEO_TOP_LMT_BLW_1000",
                    };
                    var tRunner1 = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params1);
                    if (!tRunner1.Execute()) {
                        MessageBox.error(tRunner1.GetErrorMessage());
                        return;
                    }
                    var oData1 = tRunner1.GetJSONData();
                    var SCT_REP_QNT_TEO_TOP_LMT_BLW_1000 = Number(oData1[0].Row[0].VALUE);


                    //  SCT Teorik Şifreli Limiti
                    var params2 = {
                        "Param.1": "SCT_REP_QNT_TEO_PSW_LMT",
                    };
                    var tRunner2 = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params2);
                    if (!tRunner2.Execute()) {
                        MessageBox.error(tRunner2.GetErrorMessage());
                        return;
                    }
                    var oData2 = tRunner2.GetJSONData();
                    var SCT_REP_QNT_TEO_PSW_LMT = Number(oData2[0].Row[0].VALUE);


                    //  SCT Teorik Top Limiti >1000
                    var params3 = {
                        "Param.1": "SCT_REP_QNT_TEO_TOP_LMT_OVER_1000",
                    };
                    var tRunner3 = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params3);
                    if (!tRunner3.Execute()) {
                        MessageBox.error(tRunner3.GetErrorMessage());
                        return;
                    }
                    var oData3 = tRunner3.GetJSONData();
                    var SCT_REP_QNT_TEO_TOP_LMT_OVER_1000 = Number(oData3[0].Row[0].VALUE);


                    //  SCT Ağırlık Tolerans Limiti Şifresi
                    var params4 = {
                        "Param.1": "SCT_REP_QNT_WEI_TOL",
                    };
                    var tRunner4 = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params4);
                    if (!tRunner4.Execute()) {
                        MessageBox.error(tRunner4.GetErrorMessage());
                        return;
                    }
                    var oData4 = tRunner4.GetJSONData();
                    var SCT_REP_QNT_WEI_TOL = oData4[0].Row[0].VALUE;

                    


                    if (prodTypeSCT != "SCRAP") {
                        if (parseInt(productionTypeWeight * 1000) < 1000) {
                            if (selectedProdType == "R" || selectedProdType == "M") {
                                var teoWeightTol = parseFloat(((productionTypeWeight - productionTeoWeight) / productionTeoWeight * 100).toFixed(2));
                                if ((teoWeightTol > SCT_REP_QNT_TEO_TOP_LMT_BLW_1000 || teoWeightTol < -SCT_REP_QNT_TEO_TOP_LMT_BLW_1000)) {
                                    MessageBox.error("Ağırlık tolerans limitinin dışında!(Limit: %" + SCT_REP_QNT_TEO_TOP_LMT_BLW_1000 + ")");
                                    isSaveRecords = true;
                                    return;
                                }
                                if ((teoWeightTol > SCT_REP_QNT_TEO_PSW_LMT) || (teoWeightTol < -SCT_REP_QNT_TEO_PSW_LMT)) {
                                    var adminPassword = prompt("Ağırlık tolerans limitinde!\nDevam edebilmek için şifre gerekli: ")
                                    if (adminPassword != SCT_REP_QNT_WEI_TOL) {
                            ...
                            if (selectedProdType == "R" || selectedProdType == "M") {
                                var teoWeightTol = parseFloat(((productionTypeWeight - productionTeoWeight) / productionTeoWeight * 100).toFixed(2));
                                if ((teoWeightTol > SCT_REP_QNT_TEO_TOP_LMT_OVER_1000 || teoWeightTol < -SCT_REP_QNT_TEO_TOP_LMT_OVER_1000)) {
                                    MessageBox.error("Ağırlık tolerans limitinin dışında!(Limit: %" + SCT_REP_QNT_TEO_TOP_LMT_OVER_1000 + ")");
                                    isSaveRecords = true;
                                    return;
                                }
                                if ((teoWeightTol > SCT_REP_QNT_TEO_PSW_LMT) || (teoWeightTol < -SCT_REP_QNT_TEO_PSW_LMT)) {
                    ...
                    if (prodTypeSCT != "SCRAP") {
                        ...
                        var valueCNT = Number(oDataCNT[0].Row[0].VALUE);
                        ...
                        if (!!modelArr[0]) {
                            ...
                            if (!!QTYfilter[0]) {
                                for (var i = 0; i < QTYfilter.length; i++) {
                                    var totalQTY = QTYfilter[i].TOTAL_QTY + totalQTY;
                                }
                            }
                        }
                        ...
                        if (
                            !(
                                Number((Number(totalQTY) + Number(productionTypeWeight)).toFixed(3)) <=
                                Number((Number(this.appData.selected.quantityReleased) + Number(this.appData.selected.quantityReleased) * (Number(valueCNT) / 100)).toFixed(3))
                            )
                        ) {
                            MessageBox.error(
                                "Girilen tonaj değeri toplam planlanan miktarı geçemez"
                        ...
                        var selectedRowNumber = this.getView().byId("rowNumberComboBox")?.getSelectedKey();
                        var aufnr = this.appData.selected.order.orderNo;
                        var aprio = this.appData.selected.operationNo;
                        var batchNo = this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[3].mProperties.value;
                        ...
                        if (!!selectedRowNumber) {
                            ...
                            var response = TransactionCaller.sync(
                                "MES/Itelli/DNA/OldRowNumber/updateRowNumberTrns",
                                {
                                    ...
                                    I_AUFNR: aufnr,
                                    I_BATCHNO: batchNo,
                                    I_OLDROWNUMBER: selectedRowNumber,
                                    I_NEW: "",
                                },
                                "O_JSON"
                            );
                    ...
                    if (dcElement == "SCRAP") {
                        ...
                        //productionTeoWeight = (((Math.pow(cap, 2) / 100 / 4) * 3.14159 * 7.85 * (sumQuantity * 100)) / 1000 / 1000).toFixed(3);
                        /*
                        var params = {
                            I_AUFNR: aufnr,
                        ...
                        */
                        var tonnage = productionTypeWeight;
                        quantity = tonnage;
                    ...
                    //Tamamen
                    for (var i = oData.length - 1; 0 <= i; i--) {
                    ...
                    this.screenObj.inputChar.push({
                        CHAR_NAME: "Y_METRAJ",
                        CHAR_VALUE: sumQuantity,
                    });
                    this.screenObj.inputChar.push({
                        CHAR_NAME: "Y_KGM",
                        CHAR_VALUE: (tonnage * 1000 / sumQuantity).toFixed(6),
                    });

                    var selectedBobbinNo = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[5].getSelectedKey();

                    this.screenObj.inputChar.push({
                        CHAR_NAME: "Y_FIZIKI_BOBIN_NO",
                        CHAR_VALUE: selectedBobbinNo,
                    });

                    var selectedProdType = this.getView().byId("selectProductionType").getSelectedKey();

                    var responseProdType = TransactionCaller.sync(
                        "MES/Itelli/DNA/ReportProduction/TelCekme/updateProdTypeSCTTrns",
                        {
                            I_PROD_TYPE: selectedProdType,
                            I_BATCH: newBatch,
                            I_AUFNR: aufnr,
                            I_APRIO: aprio,
                        },
                    ...
                    if (responseProdType[1] == "E") {
                        MessageBox.error(responseProdType[0]);
                        isSaveRecords = true;
                        return null;
                    }
                    ...
                    tableArr.forEach(function (item, index) {
                        ...
                        this.screenObj.inputChar.push({
                            CHAR_NAME: charName,
                            CHAR_VALUE: oData[item].Y_DOKUMNO,
                        });
                    }, this);
            ...
            getBobbinNo: function () {
                ...
                var rowNumber = Array.isArray(response[0]?.Rowsets?.Rowset?.Row) ? response[0]?.Rowsets?.Rowset?.Row : new Array(response[0]?.Rowsets?.Rowset?.Row);
                var rowNumberModel = new sap.ui.model.json.JSONModel(rowNumber);
                rowNumberModel.setSizeLimit(10000);

                this.getView().setModel(rowNumberModel, "bobbinModel");
            },
            onChangeProdunctionType: function () {
                var selectedProdType = this.getView().byId("selectProductionType").getSelectedKey();

                //  SCT Teorik Ağrılıkla İşlem
                var params = {
                    "Param.1": "SCT_REP_QNT_TEO_WEI_PSW",
                };
                var tRunner = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params);
                if (!tRunner.Execute()) {
                    MessageBox.error(tRunner.GetErrorMessage());
                    return;
                }
                var oData = tRunner.GetJSONData();
                var SCT_REP_QNT_TEO_WEI_PSW = oData[0].Row[0].VALUE;


                //  SCT Manuel Tartımla İşlem
                var params2 = {
                    "Param.1": "SCT_REP_QNT_MANUEL_WEI_PSW",
                };
                var tRunner2 = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params2);
                if (!tRunner2.Execute()) {
                    MessageBox.error(tRunner2.GetErrorMessage());
                ...
                var oData2 = tRunner2.GetJSONData();
                var SCT_REP_QNT_MANUEL_WEI_PSW = oData2[0].Row[0].VALUE;



                if (selectedProdType == "R") {  //Gerçek Ağırlık
                    console.log("R seçildi");
                    this.getView().byId("getWeightFromScale").setVisible(true);
                    this.getView().byId("idManuelInputBox").setVisible(false);
                    this.getView().byId("getManuelWeightInput").setVisible(false);
                    this.getView().byId("idShowTeoSCT").setVisible(true);
                    this.getWeightFromScale();

                } else if (selectedProdType == "T") {   //Teorik Ağırlık
                    var adminPassword = prompt("Teorik ağırlık ile ilerlemek için lütfen şifreyi girin")
                    if (adminPassword != SCT_REP_QNT_TEO_WEI_PSW) {
                        MessageBox.error("Girilen şifre hatalı!");
                        this.getView().byId("selectProductionType").setSelectedKey("R");
                        return;
                    } else {
                        this.getView().byId("getWeightFromScale").setVisible(false);
                        this.getView().byId("idManuelInputBox").setVisible(false);
                        this.getView().byId("getManuelWeightInput").setVisible(false);
                        this.getView().byId("idShowTeoSCT").setVisible(false);

                        var saglamMiktar = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[1].mProperties.value
                        var cap = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[4].getSelectedKey();
                        cap = parseFloat(cap);
                        var toplamMiktar = (((Math.pow(cap, 2) / 100 / 4) * 3.14159 * 7.85 * (saglamMiktar * 100)) / 1000 / 1000).toFixed(3);
                        productionTypeWeight = toplamMiktar;

                        this.getView().byId("idTamamenSum").setText("Güncel Miktar : 0");
                        this.getView().byId("idTamamenSum").setVisible(false);
                        this.getView().byId("idRemaining").setText(`Fark : ${toplamMiktar}`);
                        toplamMiktar = `Net Ağırlık : ${toplamMiktar}`;
                        this.getView().byId("idToplamMiktar").setText(toplamMiktar);
                        this.getView().byId("idToplamMiktar").setVisible(true);
                        this.getView().byId("idRemaining").setVisible(false);
                        this.getView().byId("idHeader").setVisible(true);
                        this.sortConsumptionOrder();
                    }
                } else if (selectedProdType == "M") {   //Manuel Tartım
                    var adminPassword = prompt("Manuel tartım ile ilerlemek için lütfen şifreyi girin")
                    if (adminPassword != SCT_REP_QNT_MANUEL_WEI_PSW) {
                        MessageBox.error("Girilen şifre hatalı!");
                        this.getView().byId("selectProductionType").setSelectedKey("R");
                        return;
                    } else {
                        this.getView().byId("getWeightFromScale").setVisible(false);
                        this.getView().byId("idManuelInputBox").setVisible(true);
                        this.getView().byId("getManuelWeightInput").setVisible(true);
                        this.getView().byId("idShowTeoSCT").setVisible(true);
                    }
                } else {    //Hatalı seçim
                    MessageBox.error("Seçilen ağırlık yöntemi hatalıdır!");
                    return;
                }
            },
            getWeightFromScale: function () {
                var response = TransactionCaller.sync(
                    "MES/Itelli/DNA/ReportProduction/TelCekme/getWeightFromSCTScaleTrns", {},
                    "O_JSON"
                );
                if (response[1] == "E") {
                    MessageBox.error(response[0]);
                    ...
                    var weightValueFromScale = response[0];
                }

                var bobbinNo = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[5].getSelectedKey();
                var bobbinsArray = this.getView().getModel("bobbinModel").getData()
                var bobbinWeight = bobbinsArray.find(item => item.BOBBINNO == bobbinNo).WEIGHT;
                weightValueFromScale = weightValueFromScale - bobbinWeight;

                weightValueFromScale = (weightValueFromScale / 1000).toFixed(3); //KG to TON

                productionTypeWeight = weightValueFromScale;

                var saglamMiktar = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[1].mProperties.value
                var cap = this.getView().byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[4].getSelectedKey();
                cap = parseFloat(cap);
                productionTeoWeight = (((Math.pow(cap, 2) / 100 / 4) * 3.14159 * 7.85 * (saglamMiktar * 100)) / 1000 / 1000).toFixed(3);
                var teoWeightSCT = productionTeoWeight;

                var weightTolerance = ((productionTypeWeight - teoWeightSCT) / teoWeightSCT * 100).toFixed(2);

                this.getView().byId("teoWeightLabel").setText(`Teorik Ağırlık : ${teoWeightSCT}`);
                this.getView().byId("bobbinWeightLabel").setText(`Bobin Ağırlığı : ${bobbinWeight}`);
                this.getView().byId("weightToleranceLabel").setText(`Fark : %${weightTolerance}`);

                this.getView().byId("idTamamenSum").setText("Güncel Miktar : 0");
                this.getView().byId("idTamamenSum").setVisible(false);
                this.getView().byId("idRemaining").setText(`Fark : ${weightValueFromScale}`);

                weightValueFromScale = `Net Ağırlık : ${weightValueFromScale}`;
                this.getView().byId("idToplamMiktar").setText(weightValueFromScale);
                this.getView().byId("idToplamMiktar").setVisible(true);
                this.getView().byId("idRemaining").setVisible(false);
                this.getView().byId("idHeader").setVisible(true);
                this.sortConsumptionOrder();
            },
            onPressManuelWeightInput: function () {
                var weightInput = this.getView().byId("idManuelInput").getValue();
                ...
                if (!!weightInput && weightInput > 0) {
                    ...
                    var bobbinsArray = this.getView().getModel("bobbinModel").getData()
                    var bobbinWeight = bobbinsArray.find(item => item.BOBBINNO == bobbinNo).WEIGHT;
                    weightInput = weightInput - bobbinWeight;
                    weightInput = (weightInput / 1000).toFixed(3) //KG to TON
                    productionTypeWeight = weightInput;
                    ...
                    this.getView().byId("idTamamenSum").setText("Güncel Miktar : 0");
                    this.getView().byId("idTamamenSum").setVisible(false);
                    this.getView().byId("idRemaining").setText(`Fark : ${weightInput}`);

                    weightInput = `Net Ağırlık : ${weightInput}`;
                    this.getView().byId("idToplamMiktar").setText(weightInput);
                    this.getView().byId("idToplamMiktar").setVisible(true);
                    this.getView().byId("idRemaining").setVisible(false);
                    this.getView().byId("idHeader").setVisible(true);
                    this.sortConsumptionOrder();
                } else {
                    MessageBox.error("Girilen ağırlık değeri hatalıdır!");
                    return;
                }
            },
            sortConsumptionOrder: function () {
                var modelData = this.getView().byId("confirmTable")?.getModel()?.getData();
                var modelLength = this.getView().byId("confirmTable")?.getModel()?.getData()?.length;
                var totalQuantity = 0;
                var tempWeight = productionTypeWeight;
                for (var i = 0; i < modelLength; i++) {
                    var totalQuantity = modelData[i]?.QUANTITY + totalQuantity;
                }
                if (totalQuantity < tempWeight) {
                    MessageBox.error("Mevcut bobinlerin ağırlığı yetersizdir. Lütfen kontrol edin.");
                    return;
                }
                for (var k = 0; k < modelLength; k++) {
                    var selectBoxId = this.byId("confirmTable").mAggregations.items[k].mAggregations.cells[1].sId;
                    modelData[k].CHOSENROW = "";
                    modelData[k].CONSUMPTIONTYPE = "10";
                    this.getView().byId(selectBoxId).setSelectedKey(null);
                }
                for (var k = modelLength - 1; k >= 0; k--) {
                    var quantity = modelData[k]?.QUANTITY;
                    if (tempWeight - quantity >= 0) {
                        tempWeight = tempWeight - quantity;
                        var selectBoxId = this.byId("confirmTable").mAggregations.items[k].mAggregations.cells[1].sId;
                        modelData[k].CHOSENROW = "X";
                        modelData[k].CONSUMPTIONTYPE = "30";
                        this.getView().byId(selectBoxId).setSelectedKey(30);
                    } else if (tempWeight != 0 && tempWeight - quantity < 0) {
                        var selectBoxId = this.byId("confirmTable").mAggregations.items[k].mAggregations.cells[1].sId;
                        modelData[k].CHOSENROW = "X";
                        modelData[k].CONSUMPTIONTYPE = "20";
                        this.getView().byId(selectBoxId).setSelectedKey(20);
                        return;
                    }
                }
            },
            sortConsumptionOrderScrap: function () {
                var modelData = this.getView().byId("confirmTable")?.getModel()?.getData();
                var modelLength = this.getView().byId("confirmTable")?.getModel()?.getData()?.length;
                var totalQuantity = 0;
                var tempWeight = this.getView().byId("reportProductionQuantityTable").mAggregations.items[1].mAggregations.cells[1].mProperties.value / 1000;
                for (var i = 0; i < modelLength; i++) {
                    var totalQuantity = modelData[i]?.QUANTITY + totalQuantity;
                }
                if (totalQuantity < tempWeight) {
            ...
            PCoMetraj: function () {
                ...
                if (objek == "3007SCTEL1") {
                    ...
                    var response = TransactionCaller.sync(
                        "MES/Itelli/DNA/Integration/SCT1/getPCoSCT1Metraj", {},
                        "O_JSON"
                    );
                    if (!!response[0]) {
                        this.getView().byId("PCoMetraj").setText(response[0].toFixed(0));
                    } else {
                        this.getView().byId("PCoMetraj").setText("Otomasyondan veri alınamıyor");
                    }
                    
                }
                else if (objek == "3007SCTEL2") {
                    var response = TransactionCaller.sync(
                        "MES/Itelli/DNA/Integration/SCT2/getPCoSCT2Metraj", {},
                        "O_JSON"
                    );
                    if (!!response[0]) {
                        this.getView().byId("PCoMetraj").setText(response[0].toFixed(0));
            ...
            capToleransCalculation: function () {


                var capValue = Number(this.appData.allCharacteristic.Y_TEL_CAP_MM_SC.VALUE);
                var capTolMAx = Number(this.appData.allCharacteristic.Y_CAP_TOL_MM_MAX.VALUE);
                var capTolMin = Number(this.appData.allCharacteristic.Y_CAP_TOL_MM_MIN.VALUE);

                var diameterArray = [];

                for (i = capValue + capTolMin - 0.01; i <= capValue + capTolMAx + 0.01; i += 0.01) {


                    var tempOBJ = {};
                    tempOBJ = { ID: i.toFixed(2).toString(), DIAMETER: i.toFixed(2).toString() };
                    diameterArray.push(tempOBJ);

                    // console.log(diameterArray);
                }


                var diameterModel = new sap.ui.model.json.JSONModel();
                diameterModel.setData(diameterArray);

                this.getView().byId("reportProductionQuantityTable").getModel().setProperty("/DIAMETERLIST", diameterModel?.oData);
            ...
            getRowNumber: function () {
                ...
                var response = TransactionCaller.sync(
                    "MES/Itelli/DNA/OldRowNumber/getAvailableRowNumbersTrns",
                    {
                        I_CLIENT: client,
                        I_PLANT: plant,
                        I_AUFNR: aufnr,
                ...
                var rowNumber = Array.isArray(response[0]?.Rowsets?.Rowset[0]?.Row) ? response[0]?.Rowsets?.Rowset[0]?.Row : new Array(response[0]?.Rowsets?.Rowset[0]?.Row);
                var rowNumberModel = new sap.ui.model.json.JSONModel(rowNumber);
                rowNumberModel.setSizeLimit(10000);

                if (!!!this.getView().byId("rowNumberComboBox")?.getSelectedKey()) {

                    this.getView().byId("rowNumberComboBox").setModel(rowNumberModel);
                    this.getView().byId("rowNumberComboBox").setSelectedKey(null);
            ...
            getProductionData: function () {
                ...
                if (!!modelArr[0]) {
                    ...
                    if (!!QTYfilter) {
                        ...
                        tempModel[0].TOTAL = totalGQ.toFixed(3);
                ...
                if (!!modelArr[0]) {
                    ...
                    if (!!QTYfilter) {
                        ...
                        tempModel[1].TOTAL = totalSCRAP.toFixed(3);
                    }
                }
                if (!!modelArr[0]) {
