sap.ui.define(
    ...
    function (
        ...
        return Controller.extend("customActivity/controller/oeeReportQuantityORME", {
            ...
            callConfirmComponentList: function (p_this, p_data) {
                ...
                rows.forEach(function (item, index) {
                    ...
                    rows[index].Y_TEL_CAP_MM_SC = Number((rows[index].Y_TEL_CAP_MM_SC).replace(',', '.')).toFixed(2);
            ...
            onChangeMeterageInput: function (oEvent) {
                ...
                var ormeMeterage = oEvent.getSource().oParent.mAggregations.cells[4].mProperties.text;
                ormeMeterage = parseFloat(ormeMeterage);

                var ormeQuantity = oEvent.getSource().oParent.mAggregations.cells[6].mProperties.text;
                ormeQuantity = parseFloat(ormeQuantity);

                var quantity = Math.floor((ormeQuantity * 1000 * meterage) / ormeMeterage) / 1000;
            ...
            onClickSaveReportedQuantity: function (movetype, material,DateTimePicker) {
            ...
            onConfirmTableSelectedChanged: function (oEvent) {
                ...
                if (oEvent.getParameter("listItems").length > 1) {
                    ...
                    var filter = model?.filter((i) => (i.CT == "20"))
                    ...
                    var selectedConsumptionType = oTable?.getModel()?.getData()[selectedRowIndex]?.CT;
            ...
            onSaveRecordsComponent: function (oEvent) {
                ...
                if (isSaveRecordsComponent) {
                    ...
                    if (this.appData.node.description == "Örme") {
                        ...
                        var tableModel = this.getView().byId("reportProductionQuantityTable").getModel().getData();
                        var sIdBatch = this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[3].sId;
                    ...
                    for (var i = 0; i < oData.length; i++) {
                        ...
                        if (oData[i].T_QUANTITY - oData[i].QUANTITY > 0) {
                            oData[i].CONSUMPTIONTYPE = "20";
                        } else if (oData[i].T_QUANTITY - oData[i].QUANTITY == 0) {
                            oData[i].CONSUMPTIONTYPE = "30";
                        }
                        ...
                        params = {
                            ...
                            I_REPORTED_QUANTITY: oData[i].METERAGE,         //22.10.21 Can Oylumlu
                    ...
                    this.onClickSaveReportedQuantity(movetype, material,DateTimePicker);
                }
            },
            ...
            getProductionData: function () {
                ...
                var responseQTY = TransactionCaller.sync(
                    "MES/Itelli/DNA/CombinedProductionORME/getProductionDataORME",
                    {
                        I_AUFNR: this.appData.selected.order.orderNo,
                    },
                    "O_JSON"
                ...
                if (!!modelArr[0]) {
                    var MCFilter = modelArr.filter((i) => (i.DC_ELEMENT == "GOOD_QUANTITY" && i.NAME == "DUMMY_MC"));
                    if (!!MCFilter[0]) {
                        tempModel[0].DUMMY_MC = MCFilter[0]?.TOTAL_QTY;
                        var totalGQ = MCFilter[0]?.TOTAL_QTY;
                        this.appData.selected.örmeTotalQuantity = totalGQ;
                        this.appData.selected.örmeRemainingQty = this.appData.selected.quantityReleased - this.appData.selected.örmeTotalQuantity;
                    }       
                }
            
                if (!!modelArr[0]) {
                    var MCFilterS = modelArr.filter((i) => (i.DC_ELEMENT == "SCRAP" && i.NAME == "DUMMY_MC"));
                    if (!!MCFilterS[0]) {
                        tempModel[1].DUMMY_MC = MCFilterS[0]?.TOTAL_QTY;
                    } 
                }
