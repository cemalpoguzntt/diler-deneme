  ...
  function (
    ...
    return Controller.extend("customActivity.controller.oeeScrapOrder", {
      ...
      onInit: function () {
        ...
        if (workcenterName == "Ark Ocağı" && this.appData.plant == "3001") {
        ...
        this.getPernrList();
        this.getVincPernrList();
        this.getSepetPernrList();
        this.getPreheatingDetails();

        this.screenObj = {};
        //this.getDate();
        // this.visibleChargeListChangeControl();
        that = this;
        /*
        ...
        */
        ...
        // if (this.appData.user.userID == "custun") {
        //   this.getView().byId("chargeLineEdit").setVisible(true);
        // } else {
        //   this.getView().byId("chargeLineEdit").setVisible(false);
        // }
      ...
      getPernrList: function () {
        ...
        var params = { "Param.1": plant };
        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/getPernrQry",
          params
        );
        tRunner.ExecuteQueryAsync(this, this.callPernrList);
      },
      callVincPernrList: function (p_this, p_data) {
        var oModel = new sap.ui.model.json.JSONModel();
        oModel.setData(p_data.Rowsets.Rowset[0]);
        p_this.getView().setModel(oModel, "vincPernrList");
      },

      getVincPernrList: function () {
        var plant = this.appData.plant;
        var params = { "Param.1": plant };
        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/getVincPernrQry",
          params
        );
        tRunner.ExecuteQueryAsync(this, this.callVincPernrList);
      },
      callSepetPernrList: function (p_this, p_data) {
        var oModel = new sap.ui.model.json.JSONModel();
        oModel.setData(p_data.Rowsets.Rowset[0]);
        p_this.getView().setModel(oModel, "sepetPernrList");
      },

      getSepetPernrList: function () {
        var plant = this.appData.plant;
        var params = { "Param.1": plant };
        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/getSepetPernrQry",
          params
        );
        tRunner.ExecuteQueryAsync(this, this.callSepetPernrList);
      },
      ...
      getMasterDetailList: function (statusCodeKey) {
        ...
        if (statusCodeKey) {
          if (statusCodeKey == 260 || statusCodeKey == 360)
            var orderBy = "DESC";
        }
        //var dateVal="";
        //if(statusCodeKey==240 || statusCodeKey==340)dateVal = this.appData.shift.startDate;
      ...
      //03.10.2022 çağrı üstün fonksiyon eklendi
      onPressUpdate_1: function (oEvent) {
        this.onChangeStatusList();
      },
      //03.10.2022 çağrı üstün fonksiyon eklendi
      ...
      getStatusList: function () {
        ...
        var tRunner = new TransactionRunner(
          ...
          // "MES/UI/ScrapOrder/Role/getRoleXquery",
          params
        );
      ...
      getLastCastIdGroup: function (charges) {
        ...
        for (var i = 0; i < oData[0].Row.length; i++) {
          multiComboboxCastNo.mProperties.selectedKeys.push(
            oData[0].Row[i].CASTID
          );
        }
...
*/
            ...
            function () {
              ...
              if (multiComboboxCastNo.getSelectedItems().length > 1) {
                var chargeGroup = this.getView()
                  ...
                  .getSelectedKey();
                var castNo =
                  multiComboboxCastNo.mProperties.selectedKeys.toString();
                //_this.calForSum(castNo);
                var plant = this.appData.plant;
                var params = {
                  I_PLANT: plant,
                  I_CASTID: castNo,
              ...
              for (var i = 0; i < items.length; i++) {
                if (i == 0) {
                  charcValue =
                    oSelect.getSelectedItems()[i].mProperties.additionalText;
                  prevCharcValue =
                    oSelect.getSelectedItems()[i].mProperties.additionalText;
                } else {
                  charcValue =
                    oSelect.getSelectedItems()[i].mProperties.additionalText;
                  prevCharcValue =
                    oSelect.getSelectedItems()[i - 1].mProperties
                      .additionalText;
                }
              ...
              if (oSelect.getSelectedItems().length > 0)
                this.screenObj.copperRate =
                  oSelect.getSelectedItems()[0].mProperties.additionalText;
          ...
          this._sTimeoutId = setTimeout(
            function () {
              ...
              if (multiCastLength > 0) {
                ...
                var castNo = "";
                var copperRate = 0;
                if (this.getView().getModel("scrapHeadModel").getData().Row[0]?.COPPER_RATE != undefined &&
                  this.getView().getModel("scrapHeadModel").getData().Row[0]?.COPPER_RATE != null &&
                  this.getView().getModel("scrapHeadModel").getData().Row[0]?.COPPER_RATE != "") {
                  copperRate = this.getView().getModel("scrapHeadModel").getData().Row[0]?.COPPER_RATE;
                }

              }

              //var chargeNo = this.getCastChargeNoList(castNo);
              //10.03.2022 --> aşağıdaki satır açıklama satırı yapıldı. Neden ilk satırı seçtirmişler?
      ...
      saveChargeScrap: function (oEvent) {
        ...
        if (
          this.screenObj.statusSelect == "330" &&
          this.screenObj.buttonText == "Ark Ocağına (Isıtmalı)"
        ) {
          this.onPressEndTemp();
        }
        ...
        if (validation == "T") {
          ...
          if (this.IsEmpty("scrPernr")) {
            this.ErrorMessage("OEE_ERROR_VOPER_NAME");

            return false;
          }
          if (this.IsEmpty("basketPernr")) {
            this.ErrorMessage("OEE_ERROR_SOPER_NAME");
        ...
        if (
          this.screenObj.statusSelect == "210" &&
          werks == "2001" &&
          chargeGroup == "YUVARLAK" &&
          chargeNo == "1"
        ) {
      ...
      callScrapHeadQry: function (p_this, p_data) {
        ...
        /*
        if((p_data.Rowsets.Rowset[0].Row[0].PLANT) == "2001"){
            if(p_data.Rowsets.Rowset[0].Row[0].BASKET_TYPE=="YUVARLAK"){
                 var basketNoSelect = p_this.getView().byId("basketNoSelect");
                  basketNoSelect.setSelectedKey("YUVARLAK");
      }
  } */
      ...
      onPressAddEntry: function (oEvent) {
        ...
        if (multiComboboxCastNo.getVisible())
          ...
          copperRate =
            this.getView().byId("masterList")._oSelectedItem.mProperties.number;
      ...
      onPressAddEntryHBI: function (oEvent) {
        ...
        var sumAllScrQuantity = this.getView()
          .byId("sumAllScrQuantity")
          .getValue();
        var totalQuantity =
          parseFloat(sumAllScrQuantity) + parseFloat(scrapQuantity);
            ...
            .getText(buttonText + " gönderme işlemini onaylıyor musunuz?"),
          {
            ...
            onClose: function (oAction) {
              if (oAction == "YES") {
                ...
                if (
                  this.screenObj.statusSelect == "220" ||
                  this.screenObj.statusSelect == "320"
                ) {
                  this.saveChargeScrap();
                }
      ...
      getCastNoSeq: function (buttonText) {
        var validation2 = this.getCastNoSeqControlCharge();
        if (validation2 == 1) {
          MessageBox.warning(
            this.getView()
              .getModel("i18n")
              .getResourceBundle()
              .getText(
                "Seçilen şarj numarasından önce gönderilmesi gereken şarj numarası var. Onaylıyor musunuz?"
              ),
            {
              actions: [MessageBox.Action.YES, MessageBox.Action.NO],
              onClose: function (oAction) {
        ...
        if (copperRateChange.IS_COPPER_RATE_CHANGE == "T") {
          MessageBox.warning(
            this.getView()
              ...
              .getResourceBundle()
              .getText(
                "Sipariş değiştirildiğinden dolayı bakır oranı farklıdır. Devam etmek istiyor musunuz? " +
                " ( Bakır Oranı: " +
                copperRateChange.FIRST_COPPER_RATE +
                " => " +
                copperRateChange.COPPER_RATE +
                " ) "
              ),
            {
              actions: [MessageBox.Action.YES, MessageBox.Action.NO],
      ...
      overTonnageControl: function () {
        ...
        var plant = this.appData.plant;
        //TÜM ŞARJLARIN TOPLAM MİKTARINA BAK
        var castNumber;
        var multiComboboxCastNo = this.getView().byId("multiComboboxCastNo");
        if (multiComboboxCastNo.getEnabled()) {
          castNumber =
            multiComboboxCastNo.getSelectedItems()[0].mProperties.key;
        }
        var castNoId = this.getView().byId("selectCastNo");
        if (castNoId.getVisible()) {
          castNumber = castNoId.getSelectedKey();
        }
        var params = {
          "Param.1": this.appData.plant,
          "Param.2": castNumber,
        ...
        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/getScrapSumQuantityAllCharge",
          params
        );
        if (!tRunner.Execute()) {
          MessageBox.error(tRunner.GetErrorMessage());
        ...
        if (statuscode == 10) {
          /*var totalQuantity = this.getView()
            ...
            .getValue(); */
          var totalQuantity = parseInt(jsData[0].Row[0].FORQUAN);
        } else {
          /*var totalQuantity = this.getView()
            .byId("sumAllScrQuantity")
            .getValue();*/
          var totalQuantity = parseInt(jsData[0].Row[0].SCRQUAN);
        }
      ...
      overTonnageScrapControl: function (scrapQuantity) {
        ...
        var params = {
          "Param.1": this.appData.plant,
          "Param.2": castNumber,
          "Param.3": this.getView().byId("chargeNoSelect").getSelectedKey(),
        };
        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/getScrapSumQuantity",
          params
        );
        if (!tRunner.Execute()) {
          MessageBox.error(tRunner.GetErrorMessage());
        ...
        if (statuscode == 10) {
          ...
          sumQuantity = parseInt(jsData[0].Row[0].FORQUAN);
        } else {
          var tonnage = 0;
          for (var i = 0; i < rows.length; i++) {
            tonnage = tonnage + parseInt(rows[i].SCRQUAN);
          }
          sumQuantity = parseInt(jsData[0].Row[0].SCRQUAN);
        }
        ...
        if (tonnage > 0 && sumQuantity) {
          tonnage = tonnage + sumQuantity;
        }
      ...
      calculateSumQuantity: function (rows) {
        ...
        for (var i = 0; i < rows.length; i++) {
          if (
            rows[i].SCRQUAN == "" ||
            JSON.stringify(parseInt(rows[i].SCRQUAN)).length >= 7
          )
            rows[i].SCRQUAN = 0;
      ...
      changeSumQuantity: function (oEvent) {
        ...
        var sumAllScrQuantity = this.getView()
          ...
          .getValue()
          .replace(".", "");
        if (Number(changeQuantity) > 165000) {
          sap.m.MessageToast.show("Tonaş Aşımı Oldu!");
          oEvent.getSource().setValue(0);
          var oData = this.getView().getModel("castDetailModel").oData;
          this.calculateSumQuantity(oData.Row);
          return;
        }
        if (Number(changeQuantity) + Number(sumAllScrQuantity) > 165000) {
          sap.m.MessageToast.show("Tonaş Aşımı Oldu!");
          oEvent.getSource().setValue(0);
          var oData = this.getView().getModel("castDetailModel").oData;
          this.calculateSumQuantity(oData.Row);
        ...
        changeQuantity = Number(changeQuantity.replace(".", ""));
        ...
        var selectedRow = oEvent
          .getSource()
          .getParent()
          .oBindingContexts.castDetailModel.sPath.split("/Row/")[1];
      ...
      getCastNoSeqControlCharge: function () {
        if (this.appData.plant == "3001"
          && this.screenObj.statusSelect != "310") {
          var params = {
            I_SCRORDID: this.screenObj.scrordid,
          ...
          var tRunner = new TransactionRunner(
            "MES/UI/ScrapOrder/Transitions/getCastNoSeqChargeXquery",
            params
          );
          if (!tRunner.Execute()) {
            MessageBox.error(tRunner.GetErrorMessage());
      ...
      calculateForSum: function (castId) {
        /*
        let allData = this.getView().getModel("masterDetailList").oData.Row

        var newArray = allData.filter(function (el) {
            return el.CASTID == castId;
        });
        */
        /*var castNumber;
        var multiComboboxCastNo = this.getView().byId("multiComboboxCastNo");
        ...
        if (tRunner.Execute()) {
          ...
          this.getView().setModel(oModel, "selectedCastData");
        } else {
          var oModel = new sap.ui.model.json.JSONModel();
          this.getView().setModel(oModel, "selectedCastData");
        }
      ...
      //Şarj Sırası Düzenle Fonksiyonu
      openChargeLineEdit: function () {
        var selectCastNo = "";
        var oView = this.getView();
        var selectCastNo = oView.byId("selectCastNo").getSelectedKey();
        if (!!selectCastNo) {
          var params = {
            "Param.1": selectCastNo,
            "Param.2": this.appData.plant
          };
          var tRunner = new TransactionRunner(
            "MES/UI/ScrapOrder/ChargeNoChange/getChargeNoQry",
            params
          );
          if (!tRunner.Execute()) {
            MessageBox.error(tRunner.GetErrorMessage());
          ...
          if (oData[0].Row == undefined) oData[0].Row = [];
          oModel.setData(oData[0].Row);
          this.getView().setModel(oModel, "chargeLineChangeModel");
          var oDialog_2 = oView.byId("scrapOrderChargeLineChange");
          if (!oDialog_2) {
            oDialog_2 = sap.ui.xmlfragment(
              oView.getId(),
              "customActivity.fragmentView.scrapOrderChargeLineChange",
              this
            );
            oView.addDependent(oDialog_2);
            //olmazsa buraya koy
          }
          oDialog_2.open();
          this.appData.oDialog_2 = oDialog_2;
          var soChargeLineChangeCastNo = this.getView().byId("soChargeLineChangeCastNo");
          soChargeLineChangeCastNo.setText(selectCastNo);
        } else {
          MessageBox.error("Lütfen Döküm Seçiniz");
          return;
        }

      },
      saveChargeLineChange: function (){
        var chargeLineChangeModel = this.getView().getModel("chargeLineChangeModel");
        var len = chargeLineChangeModel.oData.length;
        for(var i=0;i<len;i++){
          if(chargeLineChangeModel.oData[i].NEW_CHARGE_NO == undefined || chargeLineChangeModel.oData[i].NEW_CHARGE_NO == ""){
            MessageBox.error("Şarj sırası seçilmemiştir, lütfen kontrol ediniz.");
            return;
          }
          if(chargeLineChangeModel.oData[i].NEW_CHARGE_NO > len){
            MessageBox.error("Varolmayan şarj numarasın seçtiniz.");
            return;
          }
          for(var j=0; j<len;j++){
            if(i != j){
              if(chargeLineChangeModel.oData[i].NEW_CHARGE_NO == chargeLineChangeModel.oData[j].NEW_CHARGE_NO){
                MessageBox.error("Mükerrer şarj sırası seçilmiştir, lütfen kontrol ediniz.");
                return;
              }
            }
          }
        ...
        var params = {
          I_PLANT: this.appData.plant,
          I_USER: this.appData.user.userID,
          I_DATA: JSON.stringify(chargeLineChangeModel.oData)
        };

        var tRunner = new TransactionRunner(
          "MES/UI/ScrapOrder/ChargeNoChange/XQ_UPDATE_CHARGE_LINE_NO",
          params
        );
        if (!tRunner.Execute()) {
          MessageBox.error(tRunner.GetErrorMessage());
          ...
          this.appData.oDialog_2.close();
          sap.m.MessageToast.show("Sarj Sırası Değiştirme Başarılı!");
        }
      },
      handleCancel_2: function () {
        this.appData.oDialog_2.close();
      },
      visibleChargeListChangeControl: function(){
        var vControl = this.getView().byId("statusSelect").getSelectedKey();
        // if((vControl == "210" || vControl == "220" || vControl == "310" || vControl == "320") && this.getView().byId("selectCastNo").getSelectedKey() != ""){
          if((vControl == "210" || vControl == "220" || vControl == "310" || vControl == "320")){
          this.getView().byId("chargeLineEdit").setVisible(true);
        } else{
          this.getView().byId("chargeLineEdit").setVisible(false);
        }
