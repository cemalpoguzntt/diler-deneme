sap.ui.define(
  ...
  function (
    ...
    return Controller.extend("customActivity.controller.oeeSelectOrder", {
      selectedOrder: {
        orderNumber: "",
        status: "",
        routingOperNo: "",
...
getActivityOptionValues : function(obj){
  if(this.getView().getViewData().viewOptions && this.getView().getViewData().viewOptions.length>0){
    ...
    var workorder =this.selectedOrder.orderNumber;
    this.getContinuousCastingFragmentQry(workorder);

        var that = this,
          fragmentHeader = "";
        var currentDate, dateObject, startDateTemp;

if(this.appData.PODs.podId=="POD_2001_SD_01" || this.appData.PODs.podId=="POD_3001_SD_01"){
    this.checkNewQuantityQry();}
        ...
        if (this.oActivityDialog == undefined) {
          this.oActivityDialog = sap.ui.xmlfragment(
            "prodActivitiesFragment",
            "customActivity.fragmentView.activityDialog",
            this
          );
          this.byId(
      ...
      handleOK: function (oEvent) {
        ...
        if (
          ...
          orderStartResumeTime = createTimestampFromDateTimeLocal(
            startDateValue,
            startTimeValue
          );
        }
        ...
        if (
          ...
          orderEndResumeTime = createTimestampFromDateTimeLocal(
            endDateValue,
            endTimeValue
          );
        }
                    ...
                    if (this.customData.getValue() == "RESUME") {
                      ...
                      if (operationStartResumeData != undefined) {
                        ...
                        if (outputCode == 0) {
                          ...
                          //Refrakter Ömür Artış
                          if (this.appData.node.description == "Ark Ocağı" || this.appData.node.description == "Sürekli Döküm") {
                              that = this;
                              resultTable = this.orderDetails.filter(function(item){   return item.releasedHeaderID ==  that.selectedOrder.releasedHeaderID });
                              if (resultTable.length > 0) {
                                  castNmbr = resultTable[0].castingNumber;
                                  var params = {
                                      I_CAST_ID: castNmbr,
                                  ...
                                  var tRunner = new TransactionRunner(
                                  "MES/Itelli/Refractory/DataControl/XQ_Refractory",
                                  params
                                  );
                                  if (!tRunner.Execute()) {
                                      //MessageBox.error(tRunner.GetErrorMessage());
      ...
      validationCheckForEndTimeField: function () {
        ...
        if (
          ...
          endDateTime = createTimestampFromDateTimeLocal(
            endDateValue,
            endTimeValue
          );
        }
        ...
        if (
          ...
          startDateTime = createTimestampFromDateTimeLocal(
            startDateValue,
            startTimeValue
          );
        }
      ...
      openEnterTimeDialog: function (timeText) {
        ...
        if (this.oEnterTimeDialog == undefined) {
          this.oEnterTimeDialog = sap.ui.xmlfragment(
            "enterTimeFragment",
            "customActivity.fragmentView.enterTime",
            this
          );
      ...
      handleOKTime: function (oEvent) {
        ...
        if (workcenterName == "Ark Ocağı" || workcenterName == "Pota Ocağı") {
          ...
          if (row.STATUS == "FAILED") {
                ...
                onClose: function (oAction) {
                  if (oAction == "YES") {
                    ...
                    if (valueStateDate == "None" && valueStateTime == "None") {
                      try {
                        combinedDateTime =
                          createTimestampFromDateTimeLocal(
                            startDate,
                            startTime
                          );
                        if (
            ...
            if (valueStateDate == "None" && valueStateTime == "None") {
              try {
                combinedDateTime = createTimestampFromDateTimeLocal(
                  startDate,
                  startTime
                );
                if (
      ...
      onClickResume: function (resumeTime) {
        ...
        if (endDate && endTime && endDate != "" && endTime != "") {
          combinedEndDate = createTimestampFromDateTimeLocal(
            endDate,
            endTime
          );
        }
              ...
              if (stopOrderData != undefined) {
                if (stopOrderData.outputCode == 0) {
                  ...
                  if (this.appData.plant == "3001") {
                    ...
                    this.byId("idHoldStatus").setSelected(true);
                  }else{
        ...
        if (
          ...
          orderOperationJSON =
            this.interfaces.interfacesGetReleasedDemandInputForTimePeriodAndPatternAsync(
              ...
              this.appData.shift.startTimestamp,
              this.appData.shift.endTimestamp,
             [sap.oee.ui.oeeConstants.status.ACTIVE, sap.oee.ui.oeeConstants.status.NEW],
              this.appData.filtersForManageOrder.orderBasedOnCurrentShift,
              null,
              this.renderOrderDetails,
              this.getView().getController()
...
} else if (orders[0].material == "CUBUK"){
        var tRunner = new TransactionRunner(
          "MES/UI/selectOrder/getAufnrInformationHH",
          params
        );
} else {
         var tRunner = new TransactionRunner(
        ...
        if (orderOperationJSON != undefined) {
          ...
          if (orderOperationJSON.orderDetails != undefined) {
            ...
            if (
              ...
              var tRunner = new TransactionRunner(
                "MES/UI/selectOrder/getCoProductMatnrQry",
                params
              );
              if (!tRunner.Execute()) {
                MessageBox.error(tRunner.GetErrorMessage());
              ...
              for (var i = 0; i < this.orderDetails.length; i++) {
                    if(jsData[0].Row != undefined) {
                for (var k = 0; k < jsData[0].Row.length; k++) {
                  if (this.orderDetails[i].order == jsData[0].Row[k].AUFNR) {
                    this.orderDetails[i].material = jsData[0].Row[k].MATNR;
                    this.orderDetails[i].materialDescription =
                      jsData[0].Row[k].MAKTX;
                  }
                }
              }
              }
              ...
              workcenterName == "Yazıcı 2 Haddehane" ||
              workcenterName == "Çubuk Hadde Hattı - 1. Yol" ||
              workcenterName == "Çubuk Hadde Hattı - 2. Yol" ||
              workcenterName == "Tav+Hadde+Paket" ||
              workcenterName == "Tav+Hadde+Paket 2"
            ) {
              var params = {
                "Param.1": aufnrs,
              };
              var tRunner = new TransactionRunner(
                "MES/UI/selectOrder/getCoProductMatnrHHQry",
                params
              );
              if (!tRunner.Execute()) {
                MessageBox.error(tRunner.GetErrorMessage());
  ...
  /* Döküm aç ekranında silinen dökümler new statusune çekildi , ekranda görünmemesi için modelden silindi
    */
    for( var i =0; i < this.orderDetails.length; i++){ 
            if ( this.orderDetails[i].castingNumber === "NA") { 
          ...
          i=i-1;
            ...
            //Sipariş sırası geliştirmesi 09.12.2020 K.T.      -------> 22.09.2021 yorum satırı sorter ıcın açıldı.bsabak
          }
        }
      },
 ...
 var equalQuality = this.getEqualQuality();
        if (equalQuality[0].Y_KALITE_KTK != equalQuality[0].KALITE_KTK_NEW) {
          MessageBox.warning(
            this.getView()
              .getModel("i18n")
              .getResourceBundle()
              .getText("Kalite değişikliği yapıldı.Devam etmek istiyor musunuz?"),
            {
    ...
    this.selectedOrder.status != "CMPL"	
        ) {
    ...
    if(p_data.Rowsets.Rowset[0].Row[0].QUALITY_CHANGE=="T"){
            MessageBox.information("Yeni kalite mevcut : "+p_data.Rowsets.Rowset[0].Row[0].NEWQUANTITY);
    }
      },
      ...
      callcheckNewQuantityQry: function (p_this, p_data) {
    if(p_data.Rowsets.Rowset[0].Row!==undefined){
            MessageBox.information("Yeni kalite mevcut : "+p_data.Rowsets.Rowset[0].Row[0].KALITE_KTK);
    }
      },

      checkNewQuantityQry: function () {
        var params = {
          "Param.1": this.selectedOrder.orderNumber
        };
        var tRunner = new TransactionRunner(
          "MES/UI/selectOrder/getNewQuantityQry",
          params
        );
        tRunner.ExecuteQueryAsync(this, this.callcheckNewQuantityQry);
      },
...
getEqualQuality: function () {
   
        var tRunner = new TransactionRunner(
          "MES/UI/selectOrder/getEqualQualtyQry");
        if (!tRunner.Execute()) {
          MessageBox.error(tRunner.GetErrorMessage());
          return null;
        }
