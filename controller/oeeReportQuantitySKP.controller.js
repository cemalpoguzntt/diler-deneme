sap.ui.define(
    ...
    function (
        ...
        return Controller.extend("customActivity/controller/oeeReportQuantitySKP", {
            ...
            onClickConfirm: function (oEvent) {
                ...
                if (objek == "3007SKINPASS") {
                    var oView = this.getView();
                    var oDialog = oView.byId("SKPDialog");
                    if (!!oDialog) {
                        oDialog.destroy();
                        oDialog = null;
                    }
                    if (!!!oDialog) {
                        oDialog = sap.ui.xmlfragment(
                            oView.getId(),
                            "customActivity.fragmentView.repQtyDialogSKP",
                            this
                        );
                    }
                    if (!!oDialog) {
            ...
            getSKPQry: function () {
                ...
                TransactionCaller.async(
                    "MES/UI/ReportQuantity/getSKPComponentsTrns",
                    {
                        I_AUFNR: aufnr,
            ...
            onClickSaveReportedQuantity: function (movetype, material) {
                ...
                if (validDC) {
                    ...
                    if (this.appData.characteristic[0].Row[0].OBJEK == "3007SKINPASS") {
            ...
            //     var oTable = this.getView().byId("SKPDialogTable");
            ...
            onSaveSKP: function (oEvent) {
                ...
                if (isSaveSKP) {
                    ...
                    var selectedContextPath = this.getView().byId("SKPDialogTable").getSelectedContextPaths("rows")[0];
                    var selectedLineNumber = this.getView().byId("SKPDialogTable").getSelectedContextPaths("rows")[0].split("/")[1];
                    var selectedLine = this.getView().byId("SKPDialogTable").getModel().getObject(selectedContextPath);

                    var oTable = this.getView().byId("SKPDialogTable");
                    if (oTable.getSelectedItems().length == 0) {
                        sap.m.MessageToast.show(this.getView().getModel("i18n").getResourceBundle().getText("OEE_LABEL_PLEASECHOOSE"));
                        isSaveSKP = true;
                        return;
                    }
                    ...
                    if (!!!idWeight.getValue() || idWeight.getValue() == "0") {
                        sap.m.MessageToast.show(this.getView().getModel("i18n").getResourceBundle().getText("OEE_LABEL_SAVE_CONTROLE"));
                        isSaveSKP = true;
                        return;
                    }

                    var selectedWeight = this.getView().byId("SKPDialogTable").getModel().getData()[selectedLineNumber].QUANTITY
                    ...
                    scrapQuantity = selectedWeight - currentWeight;
                    ...
                    //  SKP Şifreli Hurda Girişi
                    var params = {
                        "Param.1": "SKP_REP_QNT_HURDA_PSW",
                    };
                    ...
                    var oCNTData = tRunner.GetJSONData();
                    var SKP_REP_QNT_HURDA_PSW = Number(oCNTData[0].Row[0].VALUE);


                    //  SKP Hurda Girişi
                    var params2 = {
                        "Param.1": "SKP_REP_QNT_HURDA",
                    };
                    ...
                    var oCNTData2 = tRunner2.GetJSONData();
                    var SKP_REP_QNT_HURDA = Number(oCNTData2[0].Row[0].VALUE);



                    if (oData[0].checkBox && scrapQuantity > SKP_REP_QNT_HURDA_PSW) {

                        //  SKP Hurda Girişi Şifresi
                        var params = {
                            "Param.1": "SKP_REP_QNT_HURDA_ENTRY_PSW",
                        };
                        var tRunner = new TransactionRunner("MES/Itelli/DNA/AdminPanel/selectModelbyKeyQry", params);
                        if (!tRunner.Execute()) {
                            MessageBox.error(tRunner.GetErrorMessage());
                        ...
                        var oData = tRunner.GetJSONData();
                        var value = oData[0].Row[0].VALUE;

                        var adminPassword = prompt(SKP_REP_QNT_HURDA_PSW + " kg üstünde hurda girişi yapmak için şifre gereklidir.")

                        if (adminPassword == value) {
                            this.onSaveSKP2(oEvent);
                        } else {
                            isSaveSKP = true;
                            return;
                        }

                    } else if (!oData[0].checkBox && scrapQuantity < SKP_REP_QNT_HURDA) {
                        if (confirm(scrapQuantity + " kg kalan miktar bulunuyor.\n Hurda girişi yapmadan devam etmek istiyor musunuz?")) {
                            this.onSaveSKP2(oEvent);
                        } else {
                            isSaveSKP = true;
                            return;
                    ...
                    } else if (oData[0].checkBox) {
                        if (confirm(scrapQuantity + " kg hurda olarak gönderilecek. Onaylıyor musunuz?")) {
                            this.onSaveSKP2(oEvent);
                        } else {
                            isSaveSKP = true;
                            return;
            ...
            callCheckBox: function (oEvent) {
                ...
                // if (oEvent.getParameter("listItems").length > 1) {

                //     var model = oEvent.getSource()?.getModel()?.getData();

                //     var filter = model?.filter((i) => (i.CONSUMPTIONTYPE == "20"))

                //     filter.forEach(function (item, index) {
                //         filter[index].checkBoxSelection = "X"
                //     }
                //     )

                //     model.forEach(function (item, index) {
                //         if (model[index].checkBoxSelection == "X") {
                ...
                //     var selectedRowIndex = oEvent?.getParameter("listItem")?.getBindingContextPath()?.split("/")[1];
                //     var selectedConsumptionType = oTable?.getModel()?.getData()[selectedRowIndex]?.CONSUMPTIONTYPE;



                //     if (oTable?.getItems()[selectedRowIndex]?.getSelected() == true && selectedConsumptionType == 20) {

                //         if (confirm("Parça bobin seçmek istediğinizden emin misiniz?")) {
                //             // oTable.setSelectedItem(oTable.getItems()[selectedRowIndex],true)
                //             oTable?.getItems()[selectedRowIndex]?.setSelected(true)
                //         }
                //         else {
                //             oTable?.getItems()[selectedRowIndex]?.setSelected(false)
                //         }
            ...
            onSaveSKP2: function (oEvent) {

                var objek = this.appData.characteristic[0].Row[0].OBJEK;
                var workCenter = this.appData.node.workcenterID;
                var nodeID = this.appData.node.nodeID;
                var aufnr = this.appData.selected.order.orderNo;
                var batchInput = this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[3].sId;
                var metraj = this.byId("reportProductionQuantityTable").mAggregations.items[0].mAggregations.cells[1].mProperties.value;
                var selectedContextPath = this.getView().byId("SKPDialogTable").getSelectedContextPaths("rows")[0];
                var selectedLineNumber = this.getView().byId("SKPDialogTable").getSelectedContextPaths("rows")[0].split("/")[1];
                var selectedLineModel = this.getView().byId("SKPDialogTable").getModel().getObject(selectedContextPath);
                ...
                var aprio = this.appData.selected.operationNo;
                var userID = this.appData.user.userID;
                var matnr = this.appData.selected.material.id;
                var objek = this.appData.characteristic[0].Row[0].OBJEK;
                ...
                */
                ...
                if (oData[0].checkBox) {
                    ...
                    oData[oData.length] = {};
                    oData[oData.length - 1].QUANTITY = (selectedWeight - currentWeight) / 1000;
                    ...
                    oData[oData.length - 1].CONSUMPTIONTYPE = "30";
                }

                // buna bener bi tane lazım olabilir kullanılacak
                /*
                if (objek == "3007PCSTRAND") {
                    this.screenObj.inputChar.push({
                ...
                */
                ...
                // WeightValue yukarıda yorum içerisinde 
                // oData[0].QUANTITY = parseFloat(weightValue);
                oData[0].movetype = 261;

                // Kullanılabilir
                if (oData[0].checkBox) {
                    oData[0].QUANTITY = selectedWeight / 1000; // Hurda tiklendiyse tamamını tüket

                    oData[0].CONSUMPTIONTYPE = "30";
                }
                else {
                    //Kısmen
                    if (selectedWeight - currentWeight > 0) {
                        oData[0].CONSUMPTIONTYPE = "20";
                        oData[0].QUANTITY = currentWeight / 1000;
                    }
                    //Tamamen
                ...
                oData.forEach(function (item, index) {
                    ...
                    params = {
                        ...
                        I_APRIO: aprio,
                        I_USERID: userID,
                        I_BARCODE: item.BARCODE,
                        I_QUANTITY: item.QUANTITY,
                        I_CONSUMPTIONTYPE: item?.CONSUMPTIONTYPE,
                        I_MATNR: item.MATNR,
                        I_NEWBATCH: newBatch,
                ...
                this.onClickSaveReportedQuantity(movetype, matnr);
                this.handleCancel(oEvent);
                isSaveRecords = true;
            },

            onPressGetTheoretical: function () {

                var SKPDiameter = this.appData.allCharacteristic.Y_TEL_CAP_MM_SKINNPASS.VALUE;
                if (!!!SKPDiameter) {
                    MessageBox.error("SKINPASS Çap bilgisi hatalı, kontrol ediniz.");
                    return;
                }
                ...
                if (!!!metraj) {
                    MessageBox.error("Girilen metraj bilgisi hatalı, kontrol ediniz.");
                    return;
                }

                var teoWeight = (((Math.pow(SKPDiameter, 2) / 100 / 4) * 3.14159 * 7.85 * (metraj * 100)) / 1000 / 1000).toFixed(3) * 1000;

                this.getView().byId("idWeight").setValue(teoWeight);


            },

            onPressGetWeightSKP: function () {
                var response = TransactionCaller.sync(
                ...
                if (response[1] == "E") {
                    ...
                    this.getView().byId("idWeight").setValue(response[0]);
                }
            },
